/* WardSynQ TASK 4.17: ward list -> Reports button -> all six real report envelopes appear, each
 * stating its own data source, period, generated timestamp and permission scope - never a
 * re-summarised single number this screen invented, driven in real headless Chrome over CDP
 * against the REAL ward.js and ward.css. reports.js itself is proven for real in
 * test/wardsynq-reports-bridge.test.mjs; this proves only the CLIENT half.
 *
 *   node test/run-ward-reports-golden-path.mjs   (needs Google Chrome; CHROME=... to point elsewhere)
 */
...
const PORT = 9425, userDir = (process.env.CLAUDE_JOB_DIR || "/tmp") + "/ward-reports-golden-chrome";
const CHROME = process.env.CHROME || "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";
const URL = "file://" + join(HERE, "ward-reports-golden-path-harness.html");
...
try {
  ...
  ok(await waitFor(`return window.__ready;`), "real ward.js loaded into the reports harness");

  await ev(`window.WARD.open({ orgId: "org-harness" }); return true;`);
  ok(await waitFor(`return !!document.querySelector('[data-w-act="reports"]');`), "the ward list carries a Reports button");

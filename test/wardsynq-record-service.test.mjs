...
test("role mapping: every operational role resolves to exactly the grant its capabilities imply", () => {
  ...
  assert.deepEqual(write("supervisor"), ["Patient", "Encounter", "Appointment", "PatientLink", "PrescriptionTransmission", "AdmissionRequest", "ResourceBooking", "Blackout", "RelatedPerson", "IncidentReport", "Indent", "IndentReceipt", "JobCard", "IndentDecision"]);
  assert.equal(read("supervisor"), null);
  assert.equal(tier("reception"), TIER.EXECUTE);
  assert.deepEqual(write("reception"), ["Patient", "Encounter", "Appointment", "PatientLink", "PrescriptionTransmission", "AdmissionRequest", "ResourceBooking", "Blackout", "RelatedPerson"]);
  ...
  assert.deepEqual(write("cashier"), ["Claim", "PreAuthorisation", "Invoice", "CostEstimate"]);
  ...
  assert.deepEqual(write("lab"), ["Observation", "DiagnosticReport", "SpecimenCollection", "ImagingProtocol", "CriticalResultLoop", "Indent", "IndentReceipt", "JobCard"]);
  ...
  assert.deepEqual(read("lab"), ["Patient", "ServiceRequest", "Observation", "DiagnosticReport", "SpecimenCollection", "AllergyIntolerance", "ImagingProtocol", "CriticalResultLoop", "Indent", "IndentReceipt", "JobCard", "IndentDecision", "IndentClosure", "StoreItem", "StoreLocation", "StockMovement", "Asset", "AssetEvent", "JobCardEvent"]);
...
test("OPD roles at the door: doctor writes and signs, nurse records vitals and nothing else, reception reads only, pharmacy sees orders only, hr is refused", async () => {
  ...
  assert.deepEqual(desk.descriptor.actor.writable, ["Patient", "Encounter", "Appointment", "PatientLink", "PrescriptionTransmission", "AdmissionRequest", "ResourceBooking", "Blackout", "RelatedPerson"]);

...
test("role mapping: every one of the eighteen operational roles resolves to exactly the grant its capabilities imply", () => {
  ...
  assert.deepEqual(write("nurse"), ["Observation", "ShiftHandover", "BreakGlassGrant", "Patient", "Encounter", "MedicationAdministration"]);
  ...
  for (const r of ["intern", "resident", "pg_resident"]) { assert.equal(tier(r), TIER.EXECUTE, r); assert.deepEqual(write(r), ["Observation", "ShiftHandover", "BreakGlassGrant", "Patient", "Encounter"], r); assert.equal(read(r), null, r); }
...
test("OPD roles at the door: doctor writes and signs, nurse records vitals and nothing else, reception reads only, pharmacy sees orders only, hr is refused", async () => {
  ...
  assert.deepEqual(nurse.descriptor.actor.writable, ["Observation", "ShiftHandover", "BreakGlassGrant", "Patient", "Encounter", "MedicationAdministration"]);
...
test("AI drafts: written by the AI actor on the clinician's behalf, never authored by the clinician; capped at DRAFT; no wider than the human", async () => {
  ...
  assert.equal(bot.tier, "draft"); assert.deepEqual([...bot.scope.write], ["Observation", "ShiftHandover", "BreakGlassGrant", "Patient", "Encounter", "MedicationAdministration"]); assert.equal(bot.onBehalfOf, "fb:n");
});

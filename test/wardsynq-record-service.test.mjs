...
test("role mapping: every operational role resolves to exactly the grant its capabilities imply", () => {
  ...
  assert.deepEqual(write("nurse"), ["Observation", "ShiftHandover", "BreakGlassGrant", "MedicationReconciliation", "PatientConsent", "CarePlan", "RiskAssessment", "SpecimenCollection", "Patient", "Encounter", "Appointment", "PatientLink", "PrescriptionTransmission", "MedicationAdministration"]);
...
test("OPD roles at the door: doctor writes and signs, nurse records vitals and nothing else, reception reads only, pharmacy sees orders only, hr is refused", async () => {
  ...
  assert.deepEqual(nurse.descriptor.actor.writable, ["Observation", "ShiftHandover", "BreakGlassGrant", "MedicationReconciliation", "PatientConsent", "CarePlan", "RiskAssessment", "SpecimenCollection", "Patient", "Encounter", "Appointment", "PatientLink", "PrescriptionTransmission", "MedicationAdministration"]);

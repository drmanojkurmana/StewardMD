...
test("role mapping: every operational role resolves to exactly the grant its capabilities imply", () => {
  ...
  assert.deepEqual(read("cashier"), ["MedicationOrder", "ServiceRequest", "Condition", "Claim", "PreAuthorisation", "Invoice", "CostEstimate",
    "MedicationAdministration", "DiagnosticReport", "SpecimenCollection", "MedicationDispense", "Encounter", "AmbulanceTrip", "CoverageEligibilityCheck", "PackageAssignment"]);
  ...
  assert.deepEqual(write("lab"), ["Observation", "DiagnosticReport", "SpecimenCollection", "ImagingProtocol", "CriticalResultLoop", "StockMovement", "Indent", "IndentReceipt", "JobCard"]);
  ...
  assert.deepEqual(read("lab"), ["Patient", "ServiceRequest", "Observation", "DiagnosticReport", "SpecimenCollection", "AllergyIntolerance", "ImagingProtocol", "CriticalResultLoop", "StockMovement", "Indent", "IndentReceipt", "JobCard", "IndentDecision", "IndentClosure", "StoreItem", "StoreLocation", "Asset", "AssetEvent", "JobCardEvent"]);
...
test("OPD roles at the door: doctor writes and signs, nurse records vitals and nothing else, reception reads only, pharmacy sees orders only, hr is refused", async () => {
  ...
  assert.deepEqual(desk.descriptor.actor.writable, ["Patient", "Encounter", "Appointment", "PatientLink", "PrescriptionTransmission", "AdmissionRequest", "ResourceBooking", "Blackout", "RelatedPerson", "PrivacyAcknowledgement"]);

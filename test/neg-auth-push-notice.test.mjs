...
import {
  H, KV, seedHospital, as, pushAs, admittedPatient, registerDevice, releasePotassium, loopOf, staffToken, idFor,
...
test("PUSH-04: GET /api/push/notice/<nid> answers the addressee only, audits the read, and 404s everyone else", async () => {
  ...
  assert.equal((await pushAs(null, "/notice/" + nid)).__status, 401, "no session");
  assert.equal((await pushAs(OFFDUTY, "/notice/" + nid)).__status, 404, "a member of the hospital it was not sent to");
  assert.equal((await pushAs(OTHER_DOCTOR, "/notice/" + nid)).__status, 404, "another hospital's doctor");
  assert.equal((await pushAs(LAB, "/notice/" + nid)).__status, 404, "a role that cannot read the chart");
  assert.equal((await pushAs(DOCTOR, "/notice/" + "0".repeat(32))).__status, 404, "an unknown nid");
  const otherStaff = await staffToken(OTHER_ORG, "nurse9");
  assert.equal((await pushAs(null, "/notice/" + nid, "GET", null, { "X-Staff-Token": otherStaff })).__status, 404, "another hospital's staff session");
  ...
  const ok = await pushAs(DOCTOR, "/notice/" + nid);

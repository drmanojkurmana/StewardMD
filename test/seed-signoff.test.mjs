...
test("screens: Admin > Clinical seed data marks unsigned items UNAPPROVED, shows the sign-off text for signed ones, offers sign-off only when the server says so, and a failed load says treat all as unapproved", async () => {
  ...
  const admin = html(esc, { signatory: S.SIGNATORY, canSign: false, lists });
  ...
  const owner = html(esc, { signatory: S.SIGNATORY, canSign: true, lists });
  ...
  assert.match(html(esc, { failed: true, message: "forbidden" }), /could not be loaded: forbidden\. Treat every item as UNAPPROVED/);
  assert.match(html(esc, undefined), /Loading/);

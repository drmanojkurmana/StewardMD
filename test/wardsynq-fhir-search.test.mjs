...
test("THE CAPABILITYSTATEMENT IS DERIVED FROM THE SAME TABLES THE PARSER USES", () => {
  ...
  for (const r of cs.rest[0].resource) {
    ...
    if (["Provenance", "Practitioner", "Organization", "CodeSystem", "ValueSet", "AuditEvent"].includes(r.type)) continue;
  ...
  assert.ok(cs.rest[0].resource.every((r) => ["Provenance", "Practitioner", "Organization", "CodeSystem", "ValueSet", "AuditEvent"].includes(r.type) || Object.values(FHIR_TYPE).includes(r.type)));

...
test("THE CAPABILITYSTATEMENT IS DERIVED FROM THE SAME TABLES THE PARSER USES", () => {
  ...
  for (const r of cs.rest[0].resource) {
    ...
    /* P2.5's CodeSystem, ValueSet and AuditEvent are not stored canonical types either: each has its own
     * small parser in its own file and its own test (wardsynq-fhir-terminology / -ips). */
    if (["Provenance", "Practitioner", "Organization", "CodeSystem", "ValueSet", "AuditEvent", "Subscription"].includes(r.type)) continue;
  ...
  assert.ok(cs.rest[0].resource.every((r) => ["Provenance", "Practitioner", "Organization", "CodeSystem", "ValueSet", "AuditEvent", "Subscription"].includes(r.type) || Object.values(FHIR_TYPE).includes(r.type)));

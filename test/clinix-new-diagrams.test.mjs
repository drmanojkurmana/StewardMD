...
test("diagram.auscultareas renders 5 areas, murmur radiation vectors, and connects to cardiac audio", () => {
  ...
  // Audio linkage
  assert.equal(D.soundFor("mitral"), "mr", "mitral area must link to 'mr' sound");
  assert.equal(D.soundFor("aortic"), "as", "aortic area must link to 'as' sound");
  assert.equal(D.soundFor("erbs"), "ar", "erbs area must link to 'ar' sound");

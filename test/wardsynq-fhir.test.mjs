...
test("THE CAPABILITY STATEMENT DOES NOT OVERSTATE", () => {
  ...
  assert.equal(c.rest[0].resource.length, Object.keys(FHIR_TYPE).length + 6, "it advertises exactly what it maps, plus the derived and terminology types");
  for (const r of c.rest[0].resource.filter((x) => ["CodeSystem", "ValueSet", "AuditEvent"].includes(x.type))) assert.deepEqual(r.interaction.map((i) => i.code), ["read", "search-type"], r.type + " is read-only");

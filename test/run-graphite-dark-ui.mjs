...
try{
 ...
 ok(light.toLowerCase()!=='#171719','light appearance is unchanged');
 ok(palette['--paper']==='#171719'&&palette['--panel']==='#242427','shared surfaces use Graphite');
 ok(palette['--v3-bg']==='#171719'&&palette['--v3-panel']==='#242427','home system uses Graphite');
 ok(palette['--rds-bg']==='#171719'&&palette['--rds-surface']==='#242427','module design system uses Graphite');
 ...
 ok(themes.every(x=>x[0]==='#171719'&&x[1]==='#242427'),'Graphite remains consistent across accent preferences');
 ...
 const contrast=await ev(`(()=>{const lum=h=>{const v=h.match(/[a-f0-9]{2}/gi).map(x=>parseInt(x,16)/255).map(x=>x<=.03928?x/12.92:Math.pow((x+.055)/1.055,2.4));return .2126*v[0]+.7152*v[1]+.0722*v[2]};const a=lum('f4f4f5'),b=lum('171719');return (Math.max(a,b)+.05)/(Math.min(a,b)+.05)})()`);
 ...
 ok(await ev(`getComputedStyle(document.querySelector('.sbr-set-ov')).backgroundColor==='rgb(23, 23, 25)'`),'settings screen uses Graphite');await shot('graphite-settings');
 await ev(`document.querySelector('.sbr-set-ov')?.remove();SB.openRef('syndromes')`);await sleep(100);
 ok(await ev(`getComputedStyle(document.querySelector('.kblib-tiles .kblib-f')).backgroundColor==='rgb(36, 36, 39)'`),'Knowledge Library inherits Graphite');await shot('graphite-library');
 await ev(`SB.closeRef();DX.openWorkspace()`);await sleep(100);
 ok(await ev(`getComputedStyle(document.querySelector('#dxOverlay')).backgroundColor==='rgb(23, 23, 25)'`),'clinical reasoning inherits Graphite');await shot('graphite-reasoning');
 await ev(`DX.close();const s=document.createElement('div');s.id='sknxRoot';document.body.appendChild(s)`);
 ok(await ev(`getComputedStyle(document.querySelector('#sknxRoot')).getPropertyValue('--sknx-bg').trim()==='#171719'`),'SKNX module tokens use Graphite');
 ...
 ok(JSON.stringify(localModules)===JSON.stringify(['#171719','#171719','rgb(36, 36, 39)','rgb(36, 36, 39)','#242427','#171719']),'smaller module sheets use Graphite');
 ...
 ok(maik==='#171719','MaiK uses Graphite');

...
test("the screen shows the current rules and reads them back into the shape the server enforces", () => {
  const html = A.html(c, { approvalLevels: { PurchaseOrder: 2 }, approvalPolicy: { PurchaseOrder: { approverRoles: ["admin"], expiresHours: 48, amountThresholds: [{ abovePaise: 5000000, levels: 3 }] } } });
  assert.match(html, /data-kind="PurchaseOrder"/);
  assert.match(html, /value="2"/);
  assert.match(html, /value="50000"/);
  const out = A.read(c, [row("PurchaseOrder", { levels: "1", roles: ["admin"], expires: "48", above: "50000", aboveLevels: "2" })]);
...
test("laboratory result checking shows the saved setting", () => {
  ...
  assert.match(L.html(c, { labVerification: { mode: "second-person" } }), /id="labSecond" checked/);
  assert.ok(!/id="labSecond" checked/.test(L.html(c, {})));
});

test("half-filled or impossible rules are refused on screen, not saved", () => {
  assert.match(A.read(c, [row("Invoice", { above: "100" })]).error, /both a rupee amount and a number/);
  assert.match(A.read(c, [row("Invoice", { levels: "0" })]).error, /between 1 and 5/);
  assert.match(A.read(c, [row("Invoice", { expires: "-3" })]).error, /above zero/);
});
...
test("OPD token numbering card: what is saved is exactly what the org model keeps", async () => {
  ...
  const html = K.html(c, { scope: "department", prefixes: { cardiology: "C", ghost: "G" }, deptAliases: { "gen med": "dmed" } }, depts);
  ...
  assert.match(K.html(c, undefined, depts), /value="hospital" selected/, "default is one sequence for the hospital");
  assert.match(K.html(c, {}, undefined), /Loading departments/);
  assert.match(K.html(c, {}, null), /could not be loaded[\s\S]*Do not read this as no departments/);
  const out = K.read(c, "department", [{ departmentId: "dcard", prefix: "c", aka: "Heart OPD, cardio" }, { departmentId: "dmed", name: "General Medicine", prefix: "", code: "GM", aka: "" }]);
  // D14: per department, every department needs its own prefix (typed or its code) and no two alike.
  assert.match(K.read(c, "department", [{ departmentId: "dmed", name: "General Medicine", prefix: "", code: "" }]).error, /General Medicine needs a prefix/);
  assert.match(K.read(c, "department", [{ departmentId: "a", name: "A", prefix: "GM" }, { departmentId: "b", name: "B", code: "GM" }]).error, /A and B both use the prefix GM/);
  assert.equal(K.read(c, "hospital", [{ departmentId: "dmed", name: "General Medicine", prefix: "", code: "" }]).error, undefined, "one sequence for the hospital needs no prefixes");
  ...
  assert.match(K.read(c, "department", [{ departmentId: "dcard", prefix: "TOOLONG" }]).error, /one to three letters/);
  assert.match(K.read(c, "hospital", [{ departmentId: "dcard", aka: "OPD" }, { departmentId: "dmed", aka: "opd" }]).error, /two departments/);

...
test("GET /api/queue/org/clinical-settings: 401, 403 without staff.admin, another hospital refused; the admin reads the template and what is saved", async () => {
  ...
  assert.deepEqual(r.settings, { highAlertDrugs: [], antibiotics: [], orderVerifyWithinHours: null, edReassessMinutes: {}, patientAccess: { enabled: false }, rpoMinutes: null, lactationWindowDays: null });
...
test("POST /api/queue/org/clinical-settings: the admin saves; the response is the read-back; audited in the same commit by setting name; other config untouched", async () => {
  ...
  assert.equal(r.__status, 200, JSON.stringify(r));
  assert.deepEqual(r.settings, { highAlertDrugs: ["Insulin", "Heparin"], antibiotics: ["Ceftriaxone"], orderVerifyWithinHours: 4, edReassessMinutes: { 2: 15, 3: 60 }, patientAccess: { enabled: true }, rpoMinutes: 60, lactationWindowDays: 42 });
...
test("screens: Admin > Hospital clinical settings card: loading, failed and saved read-back read differently; the form reads what the server validates", () => {
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(form)), { highAlertDrugs: ["Insulin", "Heparin"], antibiotics: [], orderVerifyWithinHours: null, rpoMinutes: 90, lactationWindowDays: null, edReassessMinutes: { 2: 15 }, patientAccess: { enabled: false } });

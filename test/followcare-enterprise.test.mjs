...
load("followcare-pathways.js"); load("followcare-integration.js"); load("followcare-intel.js");
const INT = globalThis.FollowCareIntegration, INTEL = globalThis.FollowCareIntel, PW = globalThis.FollowCarePathways;

// ---- Phase 4: integration ---------------------------------------------------------------
test("P4 diagnosisToPathway: maps free-text discharge diagnoses to pathways", () => {
  ...
  assert.equal(INT.diagnosisToPathway("something unmapped"), null);
});
...
test("P4 fromDischargeCSV: parses rows, maps diagnosis, reports unmapped/invalid", () => {
  const csv = [
    ...
    "Bad Row,123,Pneumonia,2026-07-10,MRN003",          // phone too short
    "No Dx,9876511111,Broken Leg,2026-07-10,MRN004"      // diagnosis unmapped
  ].join("\n");

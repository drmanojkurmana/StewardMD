...
test("GET /api/queue/ward/actor-names: the admin gets staff by email or staff ID and role; a mobile number, an account id or a stranger is never a name", async () => {
  ...
  assert.deepEqual(r.names[idFor(ADMIN)], { name: ADMIN, employeeId: null, role: "admin" }, "an Access sign-in, matched by the hash of the member's email");
  assert.deepEqual(r.names["fb:UIDADMIN1"], { name: ADMIN, employeeId: null, role: "admin" }, "a Google account, through its sign-in email");
  assert.deepEqual(r.names.nurse1, { name: "nurse1", employeeId: "nurse1", role: "nurse" }, "a staff ID the hospital chose");
  assert.deepEqual(r.names["8897298117"], { name: null, employeeId: null, role: "doctor" }, "a mobile number is not a name");
  assert.deepEqual(r.names[idFor(DOCTOR)], { name: null, employeeId: null, role: "doctor" }, "a member with no email: the role, and no account id as a name");
  assert.deepEqual(r.names["fb:NOBODY"], { name: null, employeeId: null, role: null });
  assert.deepEqual(r.names["someone-else"], { name: null, employeeId: null, role: null }, "nobody at this hospital is looked up anywhere else");

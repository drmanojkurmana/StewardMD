...
const SETTINGS = { pkgRoomValuation: "published_tariff", recipientOfCashlessClaims: "patient", placeOfSupply: "where_performed", intensiveCareUnits: "named_and_specialty",
...
test("could not be read is not the defaults; each setting says the chartered accountant decides and why", async () => {
  ...
  assert.equal(h.innerHTML.split("Confirm with your chartered accountant.").length - 1, 7, "six choices and the TDS schemes");
...
test("Save sends every setting and the reason; a valuation other than the published tariff shows the review's warning", async () => {
  ...
  for (const [k, v] of Object.entries({ pkgRoomValuation: "scheme_rate", recipientOfCashlessClaims: "patient", placeOfSupply: "where_performed", intensiveCareUnits: "include_hdu", roomChargeBasis: "bed_tariff", dischargeMedsAsComposite: "taxed" })) el["admGst_" + k].value = v;
  ...
  assert.deepEqual(sent[0][1], { orgId: "org-1", reason: "CA opinion received", settings: { pkgRoomValuation: "scheme_rate", recipientOfCashlessClaims: "patient", placeOfSupply: "where_performed", intensiveCareUnits: "include_hdu",
...
test("in a staff language every word of the GST settings card is translated", async () => {
  ...
  const h = host();
  await win.WSQ._renderGstSettings(ctx(win, () => Promise.resolve({ ok: true, settings: { ...SETTINGS, caOpinionRef: "CA letter 9" } })), h); await flush();

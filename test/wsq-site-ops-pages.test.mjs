...
/* GET /ward/blood-bank as blood-bank.js answers it: the Rules in force (blood-centre-rules.js bloodCentreView), a labelled
 * unit, samples, a reactive donation with its notification steps, and look-back. */
const { bloodCentreView } = await import("../functions/_wardsynq/blood-centre-rules.js");
const CENTRE = JSON.parse(JSON.stringify(bloodCentreView(null)));
const TESTS = { hiv: "non-reactive", hbv: "non-reactive", hcv: "non-reactive", syphilis: "non-reactive", malaria: "non-reactive" };
const UNIT = { unitId: "u1", unitNumber: "BAG-1-PRBC", component: "prbc", volumeMl: 280, additive: "sagm", abo: "O", rhD: "positive", collectedAt: "2026-09-15T13:00:00Z", expiresAt: "2026-10-27T00:00:00Z", storage: "2 to 6 C", status: "available",
  label: { colour: "blue", tests: [{ donationId: "do1", tti: TESTS, methods: { hiv: "elisa", hbv: "clia", hcv: "elisa", syphilis: "vdrl", malaria: "smear" }, nat: null, antibodyScreen: "positive", antibodyIdentified: "anti-M" }] } };
const BANK = () => ({ ok: true, now: "2026-09-16T00:00:00Z", questions: ["illness", "high-risk"], tti: ["hiv", "malaria"],
  criteria: CRITERIA, centre: CENTRE, components: CENTRE.components,
  ...
  donations: [{ donationId: "do1", bagNumber: "BAG-1", donorId: "dn1", volumeMl: 450, anticoagulant: "cpda", donorKind: "voluntary", adverseReaction: "Faint", collectedAt: "2026-09-15T13:00:00Z", tests: { state: "cleared", group: { abo: "O", rhD: "positive" } }, units: 1 },
    { donationId: "do2", bagNumber: "BAG-2", donorId: "dn1", volumeMl: 450, collectedAt: "2026-09-15T14:00:00Z", tests: { state: "incomplete", group: null }, units: 0 }],
  units: [UNIT, { unitId: "u2", unitNumber: "BAG-3-PLT", component: "platelets", abo: "O", rhD: "positive", expiresAt: "2026-09-18T00:00:00Z", storage: "20 to 24 C with continuous gentle agitation", status: "available", antibodyScreen: "positive", episodeId: "ep1" }],
  samples: [{ sampleId: "s1", kind: "donor-pilot", donationId: "do1", location: "Fridge 2", retainUntil: null, state: "retain" }, { sampleId: "s2", kind: "recipient", episodeId: "ep1", location: "Fridge 3", retainUntil: "2026-09-20T00:00:00Z", state: "may-discard" },
    { sampleId: "s3", kind: "donor-pilot", donationId: "do2", location: "Fridge 2", retainUntil: "2026-09-01T00:00:00Z", state: "discarded", discardedAt: "2026-09-02T00:00:00Z" }],
  notifications: [{ donationId: "do2", bagNumber: "BAG-2", reactive: ["hcv", "nat"], steps: [{ step: "notified", method: "phone", at: "2026-09-16T10:00:00Z" }, { step: "referred", referredTo: "ICTC", at: "2026-09-16T11:00:00Z" }] }],
  lookback: { reactions: [{ unitNumber: "BAG-1-PRBC", component: "prbc", patientMrn: "MRN-7", at: "2026-09-16T09:00:00Z", detail: "Chills", donations: [{ bagNumber: "BAG-1", donorNumber: "D-1" }] }],
    recall: [{ donorNumber: "D-1", bagNumber: "BAG-2", reactive: ["hcv"], earlier: [{ bagNumber: "BAG-1", collectedAt: "2026-09-15T13:00:00Z", units: [{ unitNumber: "BAG-1-PRBC", status: "issued", patientMrn: "MRN-7" }] }] }] },
...
test("blood bank: the label prints every result with its method and the group colour; samples, reactive donors and look-back read from the server; nothing reactive is offered for release", () => {
  const en = load("en"), c = ctxOf(en), B = en.win.WSQ._bloodbank, bank = BANK();
  const label = B.labelHtml(c, UNIT).replace(/&#39;/g, "'");
  assert.match(label, /background:#1f5fd6/); assert.match(label, /O RhD\+<\/span>|O RhD\+/); assert.match(label, /Blue/);
  assert.match(label, /HIV 1 and 2:<\/b> Non-reactive · ELISA/); assert.match(label, /Syphilis:<\/b> Non-reactive · VDRL/); assert.match(label, /Irregular antibody screen:<\/b> Positive/);
  assert.match(label, /Transport to the ward at 2 to 10 C\. Never freeze\./); assert.match(label, /SAGM/);
  assert.equal(B.labelHtml(c, { ...UNIT, label: undefined }), "", "no label for a unit the server does not offer one for");
  const units = B.unitsHtml(c, bank);
  assert.match(units, /data-bb="label" data-id="u1"/); assert.match(units, /data-pool="u2"/, "an available platelet unit may be pooled");
  assert.match(units, /Irregular antibody screen positive/);
  const don = B.donationsHtml(c, bank).replace(/&#39;/g, "'");
  assert.match(don, /id="bbTm-hiv"/); assert.match(don, /id="bbTt-nat"/); assert.match(don, /id="bbTtAb"/); assert.match(don, /id="bbCpAdd-prbc"/); assert.match(don, /id="bbDoAnti"/); assert.match(don, /id="bbDoKind"/);
  assert.match(don, /Tests incomplete, held in quarantine/);
  assert.match(don, /ACD: 21 days; CPDA-1: 35 days/); assert.match(don, /SAGM: 42 days/); assert.match(don, /Granulocytes<\/b> · 24 hours · 20 to 24 C/);
  assert.match(don, /NAT is optional here/); assert.match(don, /-40 C or colder \(one year is kept\)/);
  assert.match(don, /kept at least 5 years/);
  assert.match(B.donationsHtml(c, { ...bank, centre: { ...CENTRE, natRequired: true } }), /This hospital requires NAT on every donation before release/);
  const samples = B.samplesHtml(c, bank);
  assert.match(samples, /data-bb="sampleDiscard" data-id="s2"/); assert.doesNotMatch(samples, /data-id="s1"/, "a sample still needed offers no discard");
  assert.match(samples, /kept 7 days after the unit is issued/); assert.match(samples, /Discarded 2026-09-02 00:00/);
  const notify = B.notificationsHtml(c, bank);
  assert.match(notify, /Confidential\. Only blood centre staff see this/); assert.match(notify, /Hepatitis C, NAT/); assert.match(notify, /Donor told · 2026-09-16 10:00 · By phone/); assert.match(notify, /data-bb="notify" data-id="do2"/);
  assert.match(B.notificationsHtml(c, { ...bank, notifications: [] }), /No reactive donation\./);
  const lb = B.lookbackHtml(c, bank);
  assert.match(lb, /BAG-1-PRBC · Packed red cells · MRN-7/); assert.match(lb, /BAG-1-PRBC Issued MRN-7/);
  for (const f of ["samplesHtml", "notificationsHtml", "lookbackHtml"]) { assert.match(B[f](c, null), /Loading/); assert.match(B[f](c, { ok: false }), /Do not read this as no units/); }

  const adm = loadSite({ lang: "en", pages: ["bloodbank.js", "admin.js"] }), ac = ctxOf(adm);
  const card = adm.win.WSQ._bloodCentreHtml;
  assert.match(card(ac, undefined), /Loading blood centre settings/);
  assert.match(card(ac, null).replace(/&#39;/g, "'"), /Do not read this as the Rules' values being in force/);
  const form = card(ac, { ok: true, centre: { ...CENTRE, saved: { shelfHours: { platelets: 72 } } } });
  assert.match(form, /data-shelf="platelets" value="72" placeholder="120"/); assert.match(form, /id="bcNat"/); assert.match(form, /placeholder="7"/); assert.match(form, /placeholder="5"/);
  const xx = loadSite({ lang: "xx", pages: ["bloodbank.js", "admin.js"] }), xc = ctxOf(xx);
  assert.deepEqual(leftovers(xx.win.WSQ._bloodCentreHtml(xc, { ok: true, centre: CENTRE })), []);
});

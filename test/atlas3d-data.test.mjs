...
ok("66 living-CT surfaces appended after them (source flag 1): 38 organs/vessels/muscles, 27 bones, 1 body surface", live.length === 66 && parts.length === 2378 && live.every((p, k) => parts[2227 + k] === p));
...
ok("whole-body Visible Human source (flag 2): 85 parts appended after the living CT, a full-body skin and a head-to-toe skeleton (skull + cervical spine + femurs)", (() => { if (wb.length !== 85 || parts[2293] !== wb[0]) return false; const skin = wb.filter((p) => manifest.systems[p[3]].id === "integumentary"); const sk = wb.filter((p) => manifest.systems[p[3]].id === "skeletal"); const c = {}; sk.forEach((p) => { c[p[9] || "none"] = (c[p[9] || "none"] || 0) + 1; }); return skin.length === 1 && (skin[0][8][4] - skin[0][8][1]) > 1.5 && sk.length >= 40 && c.SKULL === 1 && c.CERVICAL_VERTEBRA === 7 && c.FEMUR === 2; })());
...
ok("slice planes: 24 per living-torso module plus 24 per registered Visible Human module", Object.keys(planes).length === 3 + manifest.sources[2].modules.length && manifest.sources[2].modules.length >= 6 && manifest.sources[2].modules.includes("ct-wholebody-axial") && Object.values(planes).every((m) => Object.keys(m).length === 24));
ok("whole-body parts sit inside the registered whole-body axial slab", (() => { const ax = planes["ct-wholebody-axial"]; const ys = Object.values(ax).map((p) => p.pos); const lo = Math.min(...ys) - 0.1, hi = Math.max(...ys) + 0.1; return wb.every((p) => p[8][1] >= lo && p[8][4] <= hi); })());
ok("whole-body cut planes route to the whole body, torso planes to the living CT", manifest.sources[1].modules.every((m) => /^ct-live-torso/.test(m)) && manifest.sources[2].modules.every((m) => !/^ct-live-torso/.test(m) && planes[m]));

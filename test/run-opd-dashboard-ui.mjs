...
try {
  ...
  await check('today band: seen /registered, collected, door to doctor, did not wait; changes named against the same time yesterday', async () => b.ev(`var k=[].map.call(document.querySelectorAll('[data-kpi]'),n=>n.getAttribute('data-kpi')).join(',');var seen=document.querySelector('[data-kpi="seen"]');return k==='seen,money,d2d,dnw'&&/\\//.test(seen.querySelector('.dz-suffix').textContent)&&document.querySelectorAll('[data-kpi] .dz-delta').length===3&&/4 more/.test(seen.textContent)&&/same time yesterday/.test(document.getElementById('dzKpis').textContent)&&/1 point higher/.test(document.querySelector('[data-kpi="dnw"]').textContent)||k;`));
  await check('live occupancy lists every active patient with a status pill and an action', async () => b.ev(`var rows=document.querySelectorAll('#dzOcc tbody tr');return rows.length===7&&[].every.call(rows,r=>r.querySelector('.dz-tag')&&r.querySelector('.dz-act button'))||rows.length;`));
  ...
  await check('arrivals by hour: the answer written above the bars, this hour in the accent colour, wait and consult, a table', async () => b.ev(`return /^Busiest at /.test(document.querySelector('#dzPeak .dz-lede').textContent)&&!!document.querySelector('.dz-bar.now')&&document.querySelectorAll('#dzPeak .dz-stats div').length===2&&!!document.querySelector('#dzPeak table.dz-sr')&&document.querySelector('.dz-hours-svg').getAttribute('aria-label').indexOf('Busiest')===0;`));
  await check('month: one cell per day, today ringed, nothing claimed it has no data for', async () => b.ev(`var d=document.querySelectorAll('.dz-day:not(.blank)').length;var ins=__opdUITest.st.insights;var days=new Date(Date.UTC(+ins.date.slice(0,4),+ins.date.slice(5,7),0)).getUTCDate();return d===days&&document.querySelectorAll('.dz-day.today').length===1&&!/closed/i.test(document.getElementById('dzMonth').textContent);`));
  await check('visit types: donut plus legend with counts and percent, and who went ahead ranked by reason', async () => b.ev(`var r=[].map.call(document.querySelectorAll('.dz-rank li span'),n=>n.textContent).join(',');return document.querySelectorAll('.dz-seg-f').length===3&&document.querySelectorAll('.dz-keys li').length===3&&r==='Senior citizen,Emergency,Child,Pregnant'||r;`));
  ...
  await check('a failed insights read says so on each card, with a retry, instead of "Reading..." forever', async () => b.ev(`var keep=__opdUITest.st.insights;__opdUITest.st.insights={failed:true};window.SMD_OPD_DASH.paint();var a=['dzPeak','dzMonth','dzMix'].every(id=>/could not be read\. This is not a quiet day/.test(document.getElementById(id).textContent)&&document.getElementById(id).querySelector('[data-dz-retry]'));__opdUITest.st.insights=keep;window.SMD_OPD_DASH.paint();return a&&!!document.querySelector('#dzPeak .dz-lede');`));
  await check('no card title copies the reference dashboard', async () => b.ev(`var h=[].map.call(document.querySelectorAll('#dzDash h2'),n=>n.textContent).join('|');return !/occupancy|Peak activity|authoriz|approvals/i.test(h)||h;`));

      ...
      /* ───────────────────── Conjunctivitis ───────────────────── */
      {
        ...
        assess: function (sel) {
          if (anyOf(sel, ["vision_loss", "infiltrate"]) || has(sel, "contact_lens")) return { emergency: anyOf(sel, ["vision_loss", "infiltrate"]), ladder: 4, catg: "Red eye with sight-threatening features / contact-lens wearer - treat as KERATITIS until excluded",
            ...
            abx: { firstLine: [{ drug: "Fortified cefazolin 5% (50 mg/mL) eye drops", dose: "1 drop hourly (day & night)", route: "topical (fortified)", note: "Gram-positive cover" }, { drug: "+ Fortified tobramycin 1.4% (14 mg/mL) eye drops", dose: "1 drop hourly, alternating", route: "topical (fortified)", note: "Gram-negative / Pseudomonas cover (contact-lens)" }], alt: [{ drug: "Moxifloxacin 0.5% eye drops", dose: "1 drop hourly", route: "topical", note: "fluoroquinolone monotherapy if fortified drops unavailable" }, { drug: "+ Natamycin 5% eye drops", dose: "1 drop hourly", route: "topical", note: "add if vegetative/agrarian trauma (fungal risk, India)" }], ref: "AAO Bacterial Keratitis PPP; ICMR corneal-ulcer guidance.", note: "Empiric - adjust to local antibiogram/protocol, corneal scrapings & allergy." } };
          if (has(sel, "neonate") || has(sel, "hyperacute")) return { emergency: has(sel, "hyperacute"), ladder: 3, catg: "Neonatal / hyperacute (?gonococcal or chlamydial) conjunctivitis",
            ...
            abx: { firstLine: [{ drug: "Ceftriaxone", dose: "adult 1 g single dose; neonate 25-50 mg/kg (max 125 mg) single dose", route: "systemic (IM or IV)", note: "gonococcal - SYSTEMIC, topical alone is inadequate; add saline lavage of discharge" }, { drug: "+ Azithromycin", dose: "adult 1 g single dose; neonate/infant erythromycin 12.5 mg/kg PO QID × 14 d", route: "oral", note: "covers concurrent chlamydia" }], alt: [{ drug: "Cefotaxime", dose: "neonate 100 mg/kg single dose", route: "systemic (IM/IV)", note: "preferred over ceftriaxone in jaundiced neonates / with IV calcium" }], ref: "WHO & CDC STI guidelines; AAO ophthalmia neonatorum guidance.", note: "Empiric - adjust to local antibiogram/protocol, swabs/NAAT & allergy." } };
          if (has(sel, "purulent")) return { emergency: false, ladder: 1, catg: "Likely bacterial conjunctivitis (mild)",
            ...
            abx: { firstLine: [{ drug: "Chloramphenicol 0.5% eye drops (+ 1% ointment at night)", dose: "1 drop q2-4h while awake", route: "topical", note: "broad-spectrum first line; optional - most cases self-resolve" }], alt: [{ drug: "Moxifloxacin 0.5% eye drops", dose: "1 drop TID", route: "topical", note: "or gatifloxacin 0.3%/ciprofloxacin 0.3% - reserve fluoroquinolones for severe/refractory" }], ref: "AAO Conjunctivitis PPP; ICMR AMR guidance.", note: "Empiric - adjust to local antibiogram/protocol, swabs & allergy." } };
      ...
      /* ───────────── Corneal foreign body / abrasion ───────────── */
      {
        ...
        assess: function (sel) {
          if (anyOf(sel, ["high_velocity", "penetrating"])) return { emergency: true, ladder: 5, catg: "⚠ Suspected penetrating injury / intraocular foreign body",
            ...
            abx: { firstLine: [{ drug: "Cefazolin", dose: "1 g", route: "systemic (IV) q8h", note: "open-globe endophthalmitis prophylaxis - SYSTEMIC, not topical (no drops onto an open globe)" }, { drug: "+ Ciprofloxacin", dose: "400 mg IV q12h / 750 mg PO BID", route: "systemic", note: "Gram-negative / Bacillus cover for soil-contaminated injury" }], alt: [{ drug: "Moxifloxacin", dose: "400 mg once daily", route: "systemic (IV/PO)", note: "good vitreous penetration; monotherapy option" }, { drug: "+ Vancomycin", dose: "15-20 mg/kg IV q8-12h", route: "systemic", note: "if high Gram-positive / MRSA risk" }], ref: "AAO Ocular Trauma; open-globe systemic prophylaxis. Plus tetanus prophylaxis.", note: "Empiric - adjust to local antibiogram/protocol, cultures & allergy." } };
          if (has(sel, "infiltrate")) return { emergency: true, ladder: 4, catg: "Corneal defect with infiltrate - treat as microbial keratitis",
            ...
            abx: { firstLine: [{ drug: "Fortified cefazolin 5% (50 mg/mL) eye drops", dose: "1 drop hourly", route: "topical (fortified)", note: "Gram-positive cover" }, { drug: "+ Fortified tobramycin 1.4% (14 mg/mL) eye drops", dose: "1 drop hourly, alternating", route: "topical (fortified)", note: "Gram-negative cover" }], alt: [{ drug: "Moxifloxacin 0.5% eye drops", dose: "1 drop hourly", route: "topical", note: "fluoroquinolone monotherapy alternative" }, { drug: "+ Natamycin 5% eye drops", dose: "1 drop hourly", route: "topical", note: "add if vegetative trauma / fungal risk (India)" }], ref: "AAO Bacterial Keratitis PPP; ICMR corneal-ulcer guidance.", note: "Empiric - adjust to local antibiogram/protocol, corneal scrapings & allergy." } };
          if (has(sel, "surface_fb")) return { emergency: false, ladder: 1, catg: "Superficial corneal / subtarsal foreign body",
            ...
            abx: { firstLine: [{ drug: "Chloramphenicol 0.5% eye drops / 1% ointment", dose: "QID for 3-5 days", route: "topical", note: "post-removal prophylaxis" }], alt: [{ drug: "Ciprofloxacin 0.3% or ofloxacin 0.3% eye drops", dose: "QID", route: "topical", note: "anti-pseudomonal - prefer in contact-lens wearers / deeper defects" }], ref: "AAO Corneal Abrasion / FB guidance.", note: "Empiric - adjust to local antibiogram/protocol & allergy." } };
          if (has(sel, "abrasion") || anyOf(sel, ["fb_sensation", "photophobia"])) return { emergency: false, ladder: 1, catg: has(sel, "contact_lens") ? "Corneal abrasion in a contact-lens wearer - do NOT patch (Pseudomonas risk)" : "Simple corneal abrasion",
            ...
            abx: { firstLine: [{ drug: "Chloramphenicol 0.5% eye drops / 1% ointment", dose: "QID until healed (usually 3-5 days)", route: "topical", note: "prophylaxis for a simple (non-lens) abrasion" }], alt: [{ drug: "Ciprofloxacin 0.3% or ofloxacin 0.3% eye drops", dose: "QID", route: "topical", note: "use anti-pseudomonal cover for CONTACT-LENS abrasions; do NOT patch" }], ref: "AAO Corneal Abrasion guidance.", note: "Empiric - adjust to local antibiogram/protocol & allergy." } };
      ...
      /* ───────────── Stye / chalazion ───────────── */
      {
        ...
        assess: function (sel) {
          if (has(sel, "orbital")) return { emergency: true, ladder: 3, catg: "Lid lesion with orbital signs - treat as orbital cellulitis",
            ...
            abx: { firstLine: [{ drug: "Ceftriaxone", dose: "2 g", route: "systemic (IV) q12h", note: "admit; treat as orbital cellulitis" }, { drug: "+ Vancomycin", dose: "15-20 mg/kg", route: "systemic (IV) q8-12h", note: "MRSA cover" }, { drug: "+ Metronidazole", dose: "500 mg", route: "systemic (IV) q8h", note: "anaerobic / sinus source cover" }], alt: [{ drug: "Amoxicillin-clavulanate", dose: "1.2 g", route: "systemic (IV) q8h", note: "if no MRSA/intracranial concern" }], ref: "AAO / ICMR; orbital cellulitis pathway.", note: "Empiric - adjust to local antibiogram/protocol & allergy." } };
          if (has(sel, "spreading")) return { emergency: false, ladder: 2, catg: "Stye with surrounding (preseptal) cellulitis",
            ...
            abx: { firstLine: [{ drug: "Amoxicillin-clavulanate", dose: "625 mg", route: "oral TID (child 30 mg/kg/day divided)", note: "covers the surrounding preseptal cellulitis" }], alt: [{ drug: "Cefuroxime axetil", dose: "500 mg", route: "oral BID" }, { drug: "Clindamycin or co-trimoxazole", dose: "300 mg TID / 960 mg BID", route: "oral", note: "if MRSA suspected / penicillin allergy" }], ref: "AAO / ICMR preseptal cellulitis guidance.", note: "Empiric - adjust to local antibiogram/protocol & allergy." } };
      ...
      /* ───────────── Microbial keratitis / corneal ulcer ───────────── */
      {
        ...
        assess: function (sel) {
          if (anyOf(sel, ["perforation", "scleral"])) return { emergency: true, ladder: 5, catg: "⚠ Keratitis with perforation / scleral spread - globe-threatening",
            ...
            abx: { firstLine: [{ drug: "Fortified cefazolin 5% (50 mg/mL) eye drops", dose: "1 drop hourly (day & night)", route: "topical (fortified)", note: "Gram-positive cover" }, { drug: "+ Fortified tobramycin 1.4% (14 mg/mL) eye drops", dose: "1 drop hourly, alternating", route: "topical (fortified)", note: "Gram-negative / Pseudomonas cover" }], alt: [{ drug: "Fortified vancomycin 5% eye drops", dose: "1 drop hourly", route: "topical (fortified)", note: "for resistant Gram-positive" }, { drug: "+ Natamycin 5% eye drops", dose: "1 drop hourly", route: "topical", note: "add for fungal / vegetative-trauma ulcer (India)" }], ref: "AAO Bacterial Keratitis PPP; ICMR corneal-ulcer guidance.", note: "Empiric - adjust to local antibiogram/protocol, corneal scrapings & allergy." } };
          if (anyOf(sel, ["infiltrate", "hypopyon"]) || (has(sel, "contact_lens") && has(sel, "pain"))) return { emergency: true, ladder: 4, catg: "Microbial keratitis / corneal ulcer - sight-threatening",
            ...
            abx: { firstLine: [{ drug: "Fortified cefazolin 5% (50 mg/mL) eye drops", dose: "1 drop hourly (day & night)", route: "topical (fortified)", note: "Gram-positive cover" }, { drug: "+ Fortified tobramycin 1.4% (14 mg/mL) eye drops", dose: "1 drop hourly, alternating", route: "topical (fortified)", note: "Gram-negative / Pseudomonas (contact-lens)" }], alt: [{ drug: "Moxifloxacin 0.5% eye drops", dose: "1 drop hourly", route: "topical", note: "fluoroquinolone monotherapy where fortified drops unavailable" }, { drug: "+ Natamycin 5% eye drops (± oral itraconazole/voriconazole)", dose: "1 drop hourly", route: "topical", note: "ADD antifungal for vegetative/agrarian trauma or filamentary infiltrate - common in India" }], ref: "AAO Bacterial Keratitis PPP; ICMR corneal-ulcer guidance.", note: "Empiric - adjust to local antibiogram/protocol, corneal scrapings/KOH & allergy." } };
          return { emergency: false, ladder: 4, catg: "Suspected keratitis - needs urgent slit-lamp",
            ...
            abx: { firstLine: [{ drug: "Fortified cefazolin 5% + fortified tobramycin 1.4% eye drops", dose: "1 drop each, hourly, alternating", route: "topical (fortified)", note: "start after scraping if an infiltrate is confirmed at slit-lamp" }], alt: [{ drug: "Moxifloxacin 0.5% eye drops", dose: "1 drop hourly", route: "topical", note: "fluoroquinolone monotherapy alternative; add natamycin 5% if fungal risk" }], ref: "AAO Bacterial Keratitis PPP; ICMR corneal-ulcer guidance.", note: "Empiric - adjust to local antibiogram/protocol, corneal scrapings & allergy." } };
        }
      },
      /* ───────────── Herpes simplex / dendritic keratitis ───────────── */
      {
        ...
        assess: function (sel) {
          if (anyOf(sel, ["stromal", "vision_loss", "geographic"])) return { emergency: true, ladder: 5, catg: "⚠ Herpetic stromal keratitis / keratouveitis / geographic ulcer - sight-threatening",
            ...
            abx: { firstLine: [{ drug: "Aciclovir", dose: "400 mg 5×/day (800 mg 5×/day if HZO) for 7-14 days", route: "antiviral (oral)", note: "ANTIVIRAL, not an antibiotic; topical steroid only ADDED by the specialist under antiviral cover" }, { drug: "+ Aciclovir 3% eye ointment", dose: "5×/day", route: "antiviral (topical)", note: "for epithelial component" }], alt: [{ drug: "Valaciclovir", dose: "1 g", route: "antiviral (oral) TID" }, { drug: "Ganciclovir 0.15% ophthalmic gel", dose: "5×/day", route: "antiviral (topical)" }], ref: "AAO / Herpetic Eye Disease Study (HEDS).", note: "Herpetic - antiviral (not antibiotic); dose/route per protocol & renal function." } };
          if (has(sel, "hzo")) return { emergency: false, ladder: 2, catg: "Herpes zoster ophthalmicus (V1) - systemic antiviral + eye check",
            ...
            abx: { firstLine: [{ drug: "Aciclovir", dose: "800 mg 5×/day for 7-10 days", route: "antiviral (oral)", note: "start ideally within 72 h of rash; ANTIVIRAL, not antibiotic" }], alt: [{ drug: "Valaciclovir", dose: "1 g", route: "antiviral (oral) TID" }, { drug: "Famciclovir", dose: "500 mg", route: "antiviral (oral) TID" }, { drug: "Aciclovir", dose: "10 mg/kg q8h", route: "antiviral (IV)", note: "if immunocompromised / disseminated" }], ref: "AAO Herpes Zoster Ophthalmicus guidance.", note: "Herpetic - antiviral (not antibiotic); adjust for renal function." } };
          if (anyOf(sel, ["dendrite", "recurrent", "reduced_sensation", "on_steroid"])) return { emergency: false, ladder: 1, catg: "HSV epithelial (dendritic) keratitis",
            ...
            abx: { firstLine: [{ drug: "Ganciclovir 0.15% ophthalmic gel", dose: "5×/day until healed, then TID", route: "antiviral (topical)", note: "ANTIVIRAL, not antibiotic; NO topical steroid" }, { drug: "or Aciclovir 3% eye ointment", dose: "5×/day", route: "antiviral (topical)" }], alt: [{ drug: "Aciclovir", dose: "400 mg 5×/day", route: "antiviral (oral)", note: "oral alternative/adjunct to topical" }, { drug: "Trifluridine 1% eye drops", dose: "1 drop 9×/day", route: "antiviral (topical)" }], ref: "AAO / Herpetic Eye Disease Study (HEDS).", note: "Herpetic - antiviral (not antibiotic); confirm dendrite before treating." } };
          return { emergency: false, ladder: 1, catg: "?Herpetic keratitis - confirm the pattern",
            ...
            abx: { firstLine: [{ drug: "Ganciclovir 0.15% ophthalmic gel or aciclovir 3% eye ointment", dose: "5×/day", route: "antiviral (topical)", note: "start once a dendrite is confirmed; ANTIVIRAL, never a topical steroid" }], alt: [{ drug: "Aciclovir", dose: "400 mg 5×/day", route: "antiviral (oral)", note: "oral option pending specialist confirmation" }], ref: "AAO / HEDS.", note: "Herpetic - antiviral (not antibiotic); confirm the pattern first." } };
        }
      },
      ...
      /* ───────────── Endophthalmitis ───────────── */
      {
        ...
        assess: function () {
          return { emergency: true, ladder: 4, catg: "⚠ Endophthalmitis - sight-threatening ocular emergency (assume until excluded)",
            ...
            abx: { firstLine: [{ drug: "Vancomycin", dose: "1 mg in 0.1 mL", route: "intravitreal", note: "Gram-positive cover - given at the vitreous tap, the DEFINITIVE treatment" }, { drug: "+ Ceftazidime", dose: "2.25 mg in 0.1 mL", route: "intravitreal", note: "Gram-negative cover" }], alt: [{ drug: "Amikacin", dose: "0.4 mg in 0.1 mL", route: "intravitreal", note: "replace ceftazidime if cephalosporin allergy" }, { drug: "+ Amphotericin B or voriconazole", dose: "amphotericin B 5-10 µg / voriconazole 100 µg in 0.1 mL", route: "intravitreal", note: "add antifungal for post-traumatic / endogenous / fungal seeding" }], ref: "Endophthalmitis Vitrectomy Study (EVS); AAO.", note: "Empiric - adjust to tap microscopy/culture & protocol; treat source if endogenous." } };
        }
      },
      ...
      /* ───────────── Orbital vs preseptal cellulitis ───────────── */
      {
        ...
        assess: function (sel) {
          if (has(sel, "intracranial")) return { emergency: true, ladder: 5, catg: "⚠ Orbital cellulitis with suspected intracranial / cavernous-sinus spread",
            ...
            abx: { firstLine: [{ drug: "Ceftriaxone", dose: "2 g", route: "systemic (IV) q12h", note: "add high CNS-penetrating cover for intracranial extension" }, { drug: "+ Vancomycin", dose: "15-20 mg/kg", route: "systemic (IV) q8-12h", note: "MRSA / intracranial cover" }, { drug: "+ Metronidazole", dose: "500 mg", route: "systemic (IV) q8h", note: "anaerobes from sinus source" }], alt: [{ drug: "Meropenem", dose: "1-2 g", route: "systemic (IV) q8h", note: "with vancomycin - broad cover for intracranial / cavernous-sinus spread" }], ref: "AAO / ICMR orbital cellulitis; anticoagulation for cavernous-sinus thrombosis per neuro/haematology.", note: "Empiric - adjust to local antibiogram/protocol, cultures & allergy." } };
          if (anyOf(sel, ["proptosis", "ophthalmoplegia", "vision"])) return { emergency: true, ladder: 3, catg: "⚠ Orbital (post-septal) cellulitis - sight-threatening",
            ...
            abx: { firstLine: [{ drug: "Ceftriaxone", dose: "2 g", route: "systemic (IV) q12h", note: "start today; monitor vision, pupil (RAPD), colour, movements" }, { drug: "+ Vancomycin", dose: "15-20 mg/kg", route: "systemic (IV) q8-12h", note: "MRSA cover" }, { drug: "+ Metronidazole", dose: "500 mg", route: "systemic (IV) q8h", note: "anaerobes from sinus source" }], alt: [{ drug: "Piperacillin-tazobactam", dose: "4.5 g", route: "systemic (IV) q6-8h", note: "with vancomycin - broad single-agent backbone" }, { drug: "Amoxicillin-clavulanate", dose: "1.2 g", route: "systemic (IV) q8h", note: "if no MRSA concern" }], ref: "AAO / ICMR orbital cellulitis guidance.", note: "Empiric - adjust to local antibiogram/protocol, cultures & allergy." } };
          if (has(sel, "no_orbital")) return { emergency: false, ladder: 2, catg: "Preseptal (periorbital) cellulitis - no orbital signs",
            ...
            abx: { firstLine: [{ drug: "Amoxicillin-clavulanate", dose: "625 mg (child 30 mg/kg/day divided)", route: "oral TID", note: "with CLOSE review; lower threshold to admit children / immunocompromised" }], alt: [{ drug: "Cefuroxime axetil", dose: "500 mg", route: "oral BID" }, { drug: "Clindamycin or co-trimoxazole", dose: "300 mg TID / 960 mg BID", route: "oral", note: "if MRSA suspected / penicillin allergy" }], ref: "AAO / ICMR preseptal cellulitis guidance.", note: "Empiric - adjust to local antibiogram/protocol & allergy; escalate to IV if orbital signs appear." } };
          return { emergency: false, ladder: 2, catg: "Periorbital inflammation - assess the orbit carefully",
            ...
            abx: { firstLine: [{ drug: "Amoxicillin-clavulanate", dose: "625 mg (child 30 mg/kg/day divided)", route: "oral TID", note: "only if clearly preseptal - with close review" }], alt: [{ drug: "Cefuroxime axetil", dose: "500 mg", route: "oral BID" }, { drug: "Clindamycin", dose: "300 mg", route: "oral TID", note: "MRSA / penicillin allergy" }], ref: "AAO / ICMR periorbital cellulitis guidance.", note: "Empiric - adjust to local antibiogram/protocol & allergy; any orbital sign → switch to IV regimen + CT + ENT." } };
        }
      },

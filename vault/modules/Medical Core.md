...
| 18 | **built** | `medcore/medcore-shadow.js` + the shadow half of `medcore-boot.js`. Attaches to a real `ClinicalEventBus`, debounces 60 s per patient on wall time, records statuses to a bounded ring buffer, emits nothing back, reaches nobody |

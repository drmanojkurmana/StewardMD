...
- DONE + verified: backend engine + safety (31 tests), combined/meal/correction UI, safety
  hard-interrupt + confirm gate, motion + ported Magic-UI effects, app integration (tile/dispatch/reg).
- NOT done (rest of v1 scope): basal/IOB screens, patient profiles on `SMD_CASES` + dose-history/audit
  persistence, dashboard, settings (mg/dL<->mmol/L, rounding, max-dose, institution defaults),
  insulin database + brands + selection + comparison, conversion workflows, gated DKA + pediatric,
  native rebuild.

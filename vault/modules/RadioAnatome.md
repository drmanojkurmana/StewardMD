...
- **Beta / verified:** a module is Beta (pill on its row and header, amber footer) unless `modules.json` has `verified: true`, set by hand after a radiologist's "verified" on `/validation` (`/api/validation` holds the verdicts). Verified: the 3 living-torso modules. Open radiologist asks (2026-08-19/25): name each paranasal sinus and skull bone (head CT), brain lobes on the 7T MRI, soft-tissue window + muscles (knee, pelvis axial), more detail/resolution (thorax). Rejected but kept as Beta: abdomen, foot, hand, whole body, pelvis coronal/sagittal.

...
// ...and then the ATTRIBUTE itself became the problem. Node >= 22 requires `with { type: 'json' }`
// on a JSON import, but the esbuild Cloudflare Pages runs to bundle Functions predates import
// attributes and stopped dead at the `with`:
//   ERROR Expected ";" but found "with"   connect-agent/manifest/schema.mjs:83:39
// That one parse error failed the Functions build, which failed the whole Pages build, on every push
// for weeks; Cloudflare kept serving the last good deployment, so nothing looked broken while main
// silently stopped reaching production. It never reproduced locally, because a current wrangler
// bundles with a newer esbuild that accepts the syntax.
// schema-doc.mjs is schema.json as a plain `export default` - no attribute, so Node and esbuild
// agree - generated by scripts/gen-schema-doc.mjs and held to schema.json by
// test/connect-schema-doc.test.mjs. schema.json remains the document of record.
import SCHEMA_DOC from './schema-doc.mjs';

...
(function () {
  ...
  function regionSuscHTML(lead) {
    try {
      ...
      var ctx = S ? { spec: S.specimenChain(lead.id), set: S.settingFor(lead.id) } : {};
      var seen = {}, rows = [], used = null, anyLow = false;
      orgs.forEach(function (orgName) {
        if (rows.length >= 5 || seen[orgName]) return; seen[orgName] = 1;
        var cells = [], n = 0, k = 0, combined = null;
      ...
      var strat = used ? (R_SPEC[used.spec] || used.spec) + ", " + (R_SET[used.set] || used.set) : "";
      var fallback = used && (!used.specMatch || !used.setMatch) && ctx.spec && ctx.spec.length
        ? " No " + (R_SPEC[ctx.spec[0]] || ctx.spec[0]) + (ctx.set && ctx.set !== "all" ? " " + (R_SET[ctx.set] || ctx.set) : "") + " data in this source, so " + strat + " is shown." : "";
      return '<div class="dx-region-abg" style="margin-top:10px;padding:10px 12px;border:1px solid var(--line,#E2E8F0);border-radius:12px;background:var(--panel,#fff)">' +
        '<div style="font:800 12px var(--sans,system-ui);color:var(--ink,#0F172A)">Resistance: ' + esc(nm) + ' <span style="font-weight:600;color:var(--slate-soft,#64748B)">(% resistant' + (strat ? ", " + esc(strat) : "") + ')</span></div>' +
        rows.join("") +
        '<div style="margin-top:8px;font:500 10.5px/1.4 var(--sans,system-ui);color:var(--slate-soft,#64748B)">Decision support from the active antibiogram profile' + esc(fallback) + ' Figures from fewer than 30 isolates are left out (CLSI M39). ICMR national guidance remains the baseline; check your own hospital antibiogram before prescribing.</div>' +
  ...
  window.SMD_AI = {
    ...
    research: function (question, mode, history) {
      ...
      // Evidence Review also does a PubMed retrieval, so give it a little more headroom.
  ...
  function smdInjectUIStyles() {
    ...
    var css =
      /* accordion */
      ':is(#outputArea,#abxWizard) .card.smd-acc>h2{cursor:pointer;display:flex;align-items:center;gap:8px;flex-wrap:wrap;-webkit-user-select:none;user-select:none;margin:0;position:relative}' +
      ':is(#outputArea,#abxWizard) .card.smd-acc>h2:focus-visible{outline:2px solid var(--teal,#0e6e63);outline-offset:3px;border-radius:8px}' +
      ...
      ':is(#outputArea,#abxWizard) .card.smd-acc.smd-collapsed .smd-acc-chev{transform:rotate(-90deg)}' +
      '.smd-acc-summary{flex:1 1 100%;font-size:12.5px;font-weight:500;color:var(--muted,#64748b);margin-top:3px;line-height:1.35}' +
      ':is(#outputArea,#abxWizard) .card.smd-acc:not(.smd-collapsed) .smd-acc-summary{display:none}' +
      '.smd-acc-wrap{display:grid;grid-template-rows:1fr;transition:grid-template-rows .28s ease}' +
      ':is(#outputArea,#abxWizard) .card.smd-acc.smd-collapsed .smd-acc-wrap{grid-template-rows:0fr}' +
      '.smd-acc-body{overflow:hidden;min-height:0}' +
      ':is(#outputArea,#abxWizard) .card.smd-acc.smd-collapsed .smd-acc-body{visibility:hidden}' +
      ...
      '@media (max-width:640px){#outputArea .card{margin-bottom:9px!important}:is(#outputArea,#abxWizard) .card.smd-acc>h2{padding-top:2px;padding-bottom:2px}.smd-acc-toolbar{margin-bottom:5px}}' +

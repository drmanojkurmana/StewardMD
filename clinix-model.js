...
(function () {
  ...
  function isEmbeddable(m) {
    // embeddable:true is set ONLY after the YouTube oEmbed endpoint returned 200 for the video,
  ...
  // The owner's own asset - captured or generated by them, not sourced from a third party at
  // all, so there is no external licence to verify (the same reason an inline SVG or synthesized
  // sound always clears: we made it). ownerProduced:true is a deliberate human decision, never
  // inferred - and unlike a Commons hotlink, `src` is a REPO-LOCAL path (never an external
  // http(s) URL), because this is the one hosted-file class we are actually allowed to host.
  function isOwnerProduced(m) {
    return !!(m && m.kind === "image" && m.ownerProduced === true &&
      isStr(m.src) && /^\//.test(m.src) && !/^\/\//.test(m.src) &&
      isStr(m.licence) && isStr(m.attribution));
  }
  ...
  function validateMedia(m) {
    ...
    if (m.cleared === true) {
      ...
      if (m.kind === "embed") {
      ...
      } else if (m.synth === true) {
        // Synthesized audio: generated by Web Audio at play time, so there is no file and no
        // rights holder. It names a sound model rather than a recording, and the UI must say so.
        ...
        // fields the way inline/synth/embed are - it needs either the same platform-verified
        // proof an embed needs (commonsVerified, a real Commons file-page sourceUrl), or proof
        // it is the owner's own asset (ownerProduced, a repo-local src) - not just any src.
        if (!isCommonsVerified(m) && !isOwnerProduced(m)) e.push("media.commonsVerified or media.ownerProduced required for a cleared hosted image (" + str(m.id) + ")");
      }
  ...
  var API = {
    ...
    isOwnerProduced: isOwnerProduced,

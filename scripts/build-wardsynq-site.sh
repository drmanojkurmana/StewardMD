...
for f in hospital-auth.js ward.js ward-offline.js ward-labels.js opd-pulse-model.js opd-offline-desk.js opd-live.js opd-dashboard.js opd-dashboard.css ward.css discharge.js discharge.css patient-register.js patient-register.css opd.html opd-display.html clinic-billing.html; do
  cp "$ROOT/$f" "$OUT/$f"
done

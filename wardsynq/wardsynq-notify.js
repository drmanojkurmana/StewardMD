...
class Dispatcher {
   ...
   */
  async send(payload, channelNames) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        // A throwing channel is a failed attempt, not a crash of the escalation. The other channels
        // still get their turn, because a broken pager is not a reason to skip the phone.
        attempts.push(attemptOf(name, this.now(), null, String((err && err.message) || err)));
        continue;
      }
      ...
      attempts.push(attemptOf(name, this.now(), result,

...
const HAZARDS = Object.freeze([
  ...
  },
  {
    ...
    verification: {
      ...
      tests: [
        ...
        "documents are authenticated with HMAC-SHA256, not a hand-rolled hash",
        "ADVERSARIAL: a legacy document from an earlier build is REFUSED, not honoured",
        "a document verified with the wrong key is refused",
  ...
  },
  {
    ...
    caveat: "PARTIAL, and the reason has changed. wardsynq-transport.js now ships the transport seam: a durable outbox written BEFORE any channel is attempted, a failover ladder that stops at the first CONFIRMED delivery rather than the first non-throw, three working adapters that need no vendor (an in-app ward station queue, browser notifications, an HTTP webhook), and a SweepDriver, which closes the separate defect that every monitor's sweep() was correct and nothing called it on a timer. It also separates SENT from DELIVERED from SEEN, and only a named human acknowledging moves a notice to SEEN, so a notice that reached a device and no person stays OUTSTANDING however many transports reported success. THE MONITOR IS NOW WIRED TO THE ORCHESTRATOR, which until now it was not: the orchestrator existed and nothing called it, so the chain from a NEWS2 score to a clinician's phone was assembled but not connected. wardsynq-deterioration.js is UNCHANGED - not one line of the scoring or the responder ladder was touched - because the orchestrator adapts to the plain channel shape the monitor's Dispatcher already accepts. A re-escalation raises a NEW notice at a new sequence rather than returning the page the ignored tier already had, and an acknowledgement on ANY notice stands the whole alert down and travels back to the monitor over the same single acknowledgement event, so the monitor cannot keep sweeping an alert a clinician has already taken. An escalation that cannot name its patient is refused rather than sent as something nobody could answer for. A UNIFIED ORCHESTRATOR now sits on that transport (wardsynq-orchestrator.js): one alert carries one notice id, one patient id and one clinical alert id across every channel, the escalation on every other channel stops the moment a human answers on any one of them, and an acknowledgement emits exactly ONE event (deduplicated by the event bus on a derived id) from which the patient timeline entry is written, rather than each channel writing its own version of the same clinical fact. It also adds VIEWED between DELIVERED and SEEN, because a registrar opening an alert and a registrar accepting it are different facts and the first one must not close a loop. A StewardMD MOBILE channel is wired to the push infrastructure the product already runs, and the handset itself posts a receipt when an alert actually arrives, which is the only honest route to DELIVERED: the adapter reports a push accepted by APNs or FCM as SENT and lets the ladder continue to the pager and the station, because a gateway accepting bytes says nothing about a handset that is switched off. WHAT IS NO LONGER TRUE, corrected 2026-09-05: earlier versions of this caveat said the blocker was a vendor, credentials and a contract. That was wrong as a blanket statement. StewardMD already runs APNs and FCM in production - /api/push/status returns native:true, which is exactly pushKv && (apnsConfigured || fcmConfigured) - and the product has shipped push for Lab Watch, Code Blue and FollowCare for some time. The mobile channel therefore has a real vendor, real credentials and a real delivery path, and it is the FIRST shipped adapter that reaches a clinician who is NOT already looking at a screen. Procurement is not what stands between this row and VERIFIED. WHAT ACTUALLY KEEPS IT PARTIAL, and it is enough: (1) no channel in this build has been PROVEN on real hardware. The mobile channel is tested against a mock and has never carried a real alert to a real handset, so it is UNVERIFIED in exactly the sense verify() means, and a push path that works for a guideline update is not thereby shown to work for a 3am escalation. (2) The receipt endpoints this depends on exist only on a branch and are not deployed. (3) A Debug device build registers a SANDBOX APNs token, so a production APNS_ENV will silently fail to deliver to it - which is precisely the class of failure this control exists to catch, and it has to be ruled out during verification rather than assumed. (4) There is still no pager, SMS gateway or phone system, so the ladder below mobile reaches only somebody at a screen. (5) The ESCALATION POLICY remains UNAPPROVED and belongs to the resuscitation committee, and that is a clinical blocker no amount of working transport removes. The software path existing is not the control working, and this row does not move until a real device has carried a real escalation and a named human has acknowledged it. A site must wire a real channel and PROVE it with verify(), and until a channel has carried a test message it is reported as UNVERIFIED rather than as available, because an integration that broke three weeks ago looks exactly like one that works. The NEWS2 parameter bands are the RCP's published 2017 chart rather than seed content, but the ESCALATION POLICY attached to them (response windows and responder tiers) is UNAPPROVED and belongs to the resuscitation committee. PEWS is now modelled (wardsynq-pews.js) and the paediatric refusal points at it rather than at nothing. MEOWS is now modelled (wardsynq-obstetrics.js) and the obstetric refusal points at it; qSOFA and the sepsis bundle are modelled in wardsynq-emergency.js and are argued under HAZ-TIME-01, not here. Not clinically validated and not clinically approved.",
  },
  {
    id: "HAZ-TIME-01",
  ...
  },
  {
    ...
    control: {
      ...
      // Was PARTIAL while a correction could tell the person MAKING it that later totals were wrong
      // and tell nobody who had acted on them. That chain is now complete and proven end to end:
      // flowsheet-render.js records a read when a balance is opened, wardsynq-readlog.js turns a
      // correction into a list of people, and a test walks the whole path. Every clause of the
      // stated requirement is met, so the cap comes off.
      adequacy: "full",
    ...
    verification: {
      ...
      tests: [
        ...
        "ADVERSARIAL: end to end, the consultant who prescribed on the wrong balance is told",
        "ADVERSARIAL: only readers of the SUPERSEDED version, and only BEFORE the correction",
        "ADVERSARIAL: the output is a list of PEOPLE and what each saw, not a count of totals",
        "ADVERSARIAL: the person who ACTED on it is named first",
        "ADVERSARIAL: a value merely rendered on a page is NOT a read",
        "ADVERSARIAL: the log is bounded, because one that grows forever is a dossier",
        "ADVERSARIAL: opening a value RECORDS a read, which is what lets a correction find the reader",
        "ADVERSARIAL: merely rendering rows records NOTHING",
        "ADVERSARIAL: end to end, the flowsheet reader is found when the value is corrected",
        "ADVERSARIAL: opening a balance RECORDS a read, which is the gap HAZ-FLUID-01 named",
        "ADVERSARIAL: an empty cell renders EMPTY, not as a zero and not as a dash",
        "ADVERSARIAL: a half-empty row cannot look complete",
        "ADVERSARIAL: incompleteness is in the SAME sentence as the number, not a tooltip",
        "a backfilled cell says so rather than rendering identically",
      ],
    },
    residualRisk: "reduced: the total is honest about itself, and a correction now reaches whoever read the previous figure. Unchanged and deliberate: nothing stops anyone prescribing from an incomplete balance",
    approver: "Clinical Director of Intensive Care and Lead ICU Pharmacist",
    caveat: "PARTIAL, and the reason has moved. The chain is now complete and tested end to end: recomputeAfterCorrection() says exactly which totals changed and flags a balance that crosses a line somebody prescribes against, and wardsynq-readlog.js supplies the piece that was missing, recording that a NAMED person was shown a specific VERSION of a value so a correction produces a list of people to tell rather than a count of totals. A test walks the whole path: a consultant opens the 0 to 6 hour balance at 06:12 and prescribes on it, the urine output in it is corrected from 400 mL to 40 mL at 08:00, and the consultant is notified URGENTLY because they acted on it and the balance crossed zero. The read log deliberately records only decisive reads rather than everything rendered, is retention-bounded, and states its purpose on every entry, because a log of who looked at what is also a surveillance tool and will stop people opening things if it is used as one. That gap is closed. flowsheet-render.js draws the grid and the balance and records a read when a clinician OPENS a balance, with recordActedOn() as the stronger signal for somebody who carried it into a prescription. A test walks the whole path: the consultant opens the 0 to 6 hour balance at 06:12 and prescribes on it, the urine output is corrected from 400 mL to 40 mL at 08:00, and they are found and notified URGENTLY. Merely painting the grid records nothing, because logging every cell produces a list nobody can act on. The renderer also carries the DISPLAY half of the honesty the engine enforces: an empty cell renders blank rather than as a zero or a dash, which at a glance are the same mark; a half-charted row states '3 of 6' beside itself so it cannot look complete; a backfilled entry is marked rather than rendered identically to a contemporaneous one; and the incompleteness of a total sits in the same sentence as the number rather than in a tooltip nobody opens at 08:00. Also unchanged and deliberate: nothing gates prescribing on an incomplete balance, because a system that blocked a consultant from reading a partial total would be worked around on paper, so the honesty stays advisory. Also unchanged and deliberate: nothing gates prescribing on an incomplete balance, because a system that blocked a consultant from reading a partial total would be worked around on paper, so the honesty stays advisory. Also unbuilt: nothing gates prescribing on an incomplete balance, deliberately, because a system that blocked a consultant from reading a partial total would be worked around by adding it up on paper, but that means the honesty is advisory. NOT modelled: ventilator waveforms, the pump and ventilator protocols themselves, nutrition and calorie balance, drains and stomas, pressure-area and turning charts, and any local flowsheet layout. No rendering: this is the arithmetic and the honesty about it. Not clinically validated and not clinically approved.",
  },
  {
    id: "HAZ-PAED-01",
  ...
  {
    id: "HAZ-ML-01",
    source: "LOCAL: Medical Core. Not from the spec's assurance table. See vault/modules/Medical Core.md.",
    hazard: "A learned model that predicts how closely a patient was watched, and is read as predicting the patient",
    initialRisk: "major x probable",
    requirement: "No feature may encode observation counts, intervals, charting frequency or staffing, and a frequency-only model must be trained and beaten before any model ships.",
    control: {
      kind: "banned-feature list enforced at the only place features are built, plus a capped recency",
      adequacy: "partial",
      module: "medcore/medcore-features.js",
      summary: "Sick patients are observed more often, so a count of observations or the gap between them predicts deterioration in retrospective data better than most physiology: what it predicts is the nurse who was worried, recorded in the timestamps. BANNED_FEATURES names each shortcut with the reason it is one, and assertNoBannedFeatures() runs inside features() on EVERY call rather than in a lint nobody runs. The single surviving recency signal is a per-parameter age capped at 720 minutes, because 'this value is four hours old' is a property of the value while the uncapped interval is a description of the ward's staffing.",
    },
    verification: {
      file: "test/medcore-shortcut.test.mjs",
      tests: [
        "shortcut: no produced feature is a count, an interval or a frequency",
        "shortcut: the enforcement is a throw, not a lint nobody runs",
        "shortcut: charting the SAME patient more often must not change the feature vector much",
        "shortcut: recency is capped, so it cannot stand in for the observation interval",
  ...
  },
  {
    id: "HAZ-ML-02",
    source: "LOCAL: Medical Core. Not from the spec's assurance table. See vault/modules/Medical Core.md.",
    hazard: "The treatment paradox: a model taught that being on a treatment predicts not needing it",
    initialRisk: "catastrophic x occasional",
    requirement: "A patient already receiving the treatment at t0 must be excluded from the risk set, never labelled negative, and the preparation window before the event must not contribute features.",
    control: {
      kind: "risk-set exclusion and a declared blanking window, shared by the trainer and the bedside",
      adequacy: "partial",
    ...
    verification: {
      ...
      tests: [
        "labels: a patient already on a vasopressor is EXCLUDED from MC-3, never a negative",
        "labels: the same rule holds for ventilation and for renal replacement",
        "labels: nobody charted it is a refusal, not a no",
        "labels: an outcome that needs a resuscitation status will not be asked without one",
        "labels: riskSet and the bedside share one function, so they cannot drift",
        "labels: the blanking window drops the hour before the event, and nothing else",
        "labels: the blanking window is a declared constant, identical across outcomes",
      ],
    },
  ...
  },
  {
    id: "HAZ-ML-03",
    source: "LOCAL: Medical Core. Not from the spec's assurance table. See vault/modules/Medical Core.md.",
    hazard: "A clinical value silently converted into a different number and then scored",
    initialRisk: "major x occasional",
    requirement: "Only an explicitly listed (parameter, unit) pair may be converted; anything else must become a refusal that keeps the raw value.",
    control: {
      kind: "unit allow-list with exact factors, plus plausibility bounds as a second net",
      adequacy: "full",
      module: "medcore/medcore-units.js",
      summary: "wardToSI in icu.js converts an SI-labelled ward result BACK to conventional units despite its name, which is why the GHIS adapter refuses to normalise at all and keeps sourceValue and sourceUnit verbatim. Medical Core is where normalisation finally happens, once, against a table a clinician can read. There is no heuristic and no fallback: an unlisted unit is UNIT_UNKNOWN and the value becomes null. An unlabelled value is read only from a source whose convention is documented in this repo, and is then flagged unitAssumed so nothing downstream can treat it as labelled. Plausibility is the second net and catches the dangerous case the allow-list cannot: a creatinine measured in umol/L but LABELLED mg/dL passes the list and fails 0.1 to 25 mg/dL.",
    },
    verification: {
      file: "test/medcore-units.test.mjs",
      tests: [
        "units: an unlisted unit is refused, never guessed",
        "units: the second net catches a value whose LABEL is in the table but whose magnitude is not",
        "units: an unlabelled value is read only from a source whose convention is written down",
        "units: blank is absence, not zero",
        "units: an unknown parameter is refused rather than passed through",
        "units: mEq/L and mmol/L are the same number for a monovalent ion, and only for one",
      ],
    },
    residualRisk: "reduced: a conversion that is not in the table cannot happen, and one that is in the table but wrong in magnitude is caught by the bounds",
    approver: "Laboratory Director and the Critical Care lead",
    caveat: "IMPLEMENTED and TESTED, not clinically validated or approved. The table itself is UNAPPROVED seed content: the conversion factors are standard but nobody has signed them, and the plausibility bounds are engineering limits for catching a unit error, deliberately far wider than any clinical threshold. Coverage is 29 parameters and no more; a lab outside them is simply not read. Nothing consumes these values in a clinical path today.",
  },
  {
    id: "HAZ-ML-04",
    source: "LOCAL: Medical Core. Not from the spec's assurance table. See vault/modules/Medical Core.md.",
    hazard: "A date of birth read as an age, or an age read from a field that is not one",
    initialRisk: "major x occasional",
    requirement: "Age must come only from a reported age; no Medical Core code may perform date-of-birth arithmetic.",
    control: {
      kind: "structural absence: there is no dob code path at all",
      adequacy: "full",
      module: "medcore/medcore-state.js",
      summary: "GHIS sends age in years as a string, so the adapter records ageYears and leaves dob as the sentinel 0000-00-00, which deliberately does not parse. Age drives weight-based paediatric dosing, the 70 kg default in icu.js and every paediatric refusal, so a dob parsed into the year 45 is a dosing hazard rather than a display bug, and vault/modules/Insulin.md records that this family of bug has bitten before. readAge() reads ageYears or age, rejects anything outside 0 to 120, and rejects a non-primitive that would coerce (Number([]) is 0). The control is negative and a test enforces it: no file in medcore/ mentions dob outside a comment.",
    },
    verification: {
      file: "test/medcore-age.test.mjs",
      tests: [
        "age: the sentinel dob yields no age at all, never an age near 1981",
        "age: a real-looking dob is still not read - only ageYears is",
        "age: GHIS sends the age as a string, and that is the field that is read",
        "age: a value that cannot be an age is dropped, not clamped",
        "age: the ICU adapter reads patient.age and ignores any dob beside it",
        "age: no Medical Core module performs date-of-birth arithmetic",
      ],
    },
    residualRisk: "reduced: an age that cannot be established is absent, and an absent age makes the outcomes that need one unaskable rather than defaulted",
    approver: "Clinical Director and the Paediatrics lead",
    caveat: "IMPLEMENTED and TESTED, not clinically validated or approved. It guarantees Medical Core never invents an age; it does not guarantee the age the source system sent is correct, which no code here can check. The paediatric refusals themselves belong to wardsynq-pews.js and wardsynq-deterioration.js and are unchanged.",
  },
]);

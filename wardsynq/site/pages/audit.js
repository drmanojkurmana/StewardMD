...
(function () {
  ...
  function loadChanges(c, card, since) {
    ...
    return c.api("/" + tenantId + "/changes?since=" + (since || 0) + "&limit=100", undefined, "/api/wardsynq").then(function (r) {
      ...
      card.innerHTML = "<h2>" + title + "</h2>" +
        (records.length ? '<div class="tbl"><table><thead><tr><th>' + c.esc(T(c, "site.audit.colSeq", "Seq")) + '</th><th>' + c.esc(T(c, "site.audit.colWhen", "When")) + '</th><th>' + c.esc(T(c, "site.audit.colType", "Type")) + '</th><th>' + c.esc(T(c, "site.audit.colId", "Id")) + '</th><th>' + c.esc(T(c, "site.audit.colVersion", "Version")) + '</th><th>' + c.esc(T(c, "site.audit.colActor", "Actor")) + '</th></tr></thead><tbody>' +
          records.map(function (rec) {
            var actor = (rec.writtenBy && rec.writtenBy.id) || "";
            var at = (rec.writtenBy && rec.writtenBy.at) || "";
            return "<tr><td>" + EN(c, c.esc(rec.seq)) + "</td><td>" + EN(c, c.esc(at)) + "</td><td>" + EN(c, c.esc(rec.resourceType)) + '</td><td class="mono">' + EN(c, c.esc(rec.id)) + "</td><td>" + EN(c, c.esc(rec.version)) + "</td><td>" + EN(c, c.esc(actor)) + "</td></tr>";
          }).join("") + "</tbody></table></div>" : '<p class="quiet">' + c.esc(T(c, "site.audit.changesNone", "No changes recorded yet.")) + '</p>') +
        (records.length ? '<button type="button" class="btn quiet" id="audChangesMore">' + c.esc(T(c, "site.audit.loadMore", "Load more")) + '</button>' : "");
    ...
    return Promise.all([
      ...
      if (!log || !log.ok) {
        ...
        html += "<h3>" + c.esc(T(c, "site.audit.declarationsHeading", "Declarations")) + "</h3>" + (acts.length ? '<div class="tbl"><table><thead><tr><th>' + c.esc(T(c, "site.audit.colState", "State")) + '</th><th>' + c.esc(T(c, "site.audit.colDeclaredBy", "Declared by")) + '</th><th>' + c.esc(T(c, "site.audit.colAt", "At")) + '</th><th>' + c.esc(T(c, "site.audit.colReason", "Reason")) + '</th></tr></thead><tbody>' +
          acts.map(function (a) {
            return "<tr><td><span class=\"pill" + (a.active ? " warn" : "") + "\">" + c.esc(declState(c, a.active, a.revokedAt)) + "</span></td><td>" + EN(c, c.esc(a.declaredBy)) + "</td><td>" + EN(c, c.esc(a.declaredAt)) + "</td><td>" + EN(c, c.esc(a.reason)) + "</td></tr>";
      ...
      if (!grants || !grants.ok) {
        ...
        html += "<h3>" + c.esc(T(c, "site.audit.breakGlassHeading", "Break-glass grants")) + "</h3>" + (gl.length ? '<div class="tbl"><table><thead><tr><th>' + c.esc(T(c, "site.audit.colPatient", "Patient")) + '</th><th>' + c.esc(T(c, "site.audit.colBy", "By")) + '</th><th>' + c.esc(T(c, "site.audit.colAt", "At")) + '</th><th>' + c.esc(T(c, "site.audit.colReason", "Reason")) + '</th><th>' + c.esc(T(c, "site.audit.colNotified", "Notified")) + '</th></tr></thead><tbody>' +
          gl.map(function (g) {
            return '<tr><td class="mono">' + EN(c, c.esc(g.patientId)) + "</td><td>" + EN(c, c.esc(g.actorId)) + "</td><td>" + EN(c, c.esc(g.grantedAt)) + "</td><td>" + EN(c, c.esc(g.reason)) + "</td><td>" + c.esc(notifiedWord(c, g)) + "</td></tr>";
  ...
  function loadHealth(c, card) {
    ...
    return c.api("/ward/operational-health?orgId=" + encodeURIComponent(c.state.orgId)).then(function (r) {
      ...
      card.innerHTML = "<h2>" + title + "</h2><p class=\"quiet\">" + c.esc(T(c, "site.audit.asOfLabel", "As of")) + " " + EN(c, c.esc(h.generatedAt)) + "</p>" +
        '<h3>' + c.esc(T(c, "site.audit.aiHeading", "AI")) + '</h3><div class="kv"><dt>' + c.esc(T(c, "site.audit.statusLabel", "Status")) + '</dt><dd>' + EN(c, c.esc(ai.status)) + "</dd>" +
        ...
        '<h3>' + c.esc(T(c, "site.audit.notificationsHeading", "Notifications")) + '</h3><div class="kv"><dt>' + c.esc(T(c, "site.audit.statusLabel", "Status")) + '</dt><dd>' + EN(c, c.esc(notif.status)) + "</dd>" +
        ...
        '<h3>' + c.esc(T(c, "site.audit.digitalTwinHeading", "Digital Twin")) + '</h3><div class="kv"><dt>' + c.esc(T(c, "site.audit.statusLabel", "Status")) + '</dt><dd>' + EN(c, c.esc(twin.status)) + "</dd>" +

/* kardiox-flags.js — KardioX AI · central feature-flag registry.
 *
 * Sibling of fundx-flags.js (SMD_FUNDX_FLAGS). One source of truth for every KardioX flag: key, type,
 * default, ?query alias, description. Resolution order per flag: ?query param (if defined) wins, else
 * localStorage, else the default.
 *
 * ADDITIVE + non-breaking: defining this namespace changes nothing on its own. KardioX is gated by
 * `smd_kardiox` (DEFAULT OFF) exactly like FundX's `smd_fundx`; when off, kardiox.js returns early and
 * the whole module is a no-op. Exposed as window.SMD_KARDIOX_FLAGS.
 */
(function () {
  "use strict";

  // type: bool | int | tri (true/false/null) | enum. def: default when unset. query: ?alias (or null).
  var DEFS = {
    smd_kardiox:            { type: "bool", def: true,      query: "kardiox",     desc: "KardioX AI master flag (home card + module). DEFAULT OFF." },
    smd_kardiox_cloud:      { type: "tri",  def: null,      query: null,          desc: "Cloud ECG-analysis consent (null = ask once). Off = mock/offline only." },
    smd_kardiox_confidence: { type: "bool", def: true,      query: null,          desc: "Always show the AI confidence % (Settings · Intelligence)." },
    smd_kardiox_haptics:    { type: "bool", def: true,      query: null,          desc: "Haptic feedback for taps / report-ready / urgent / quiz." },
    smd_kardiox_dev:        { type: "bool", def: false,     query: "kardioxdev",  desc: "Developer overlay (pipeline stages, provider, timings)." },
    smd_kardiox_backend:    { type: "bool", def: false,     query: "kardioxbackend", desc: "Use the live KardioX pipeline backend (RemoteAnalyzer via /api/kardiox). Health-gated. DEFAULT OFF — an ungated cloud path (no 'is this an ECG?' input gate); off until the server model rejects non-ECG / real-photo inputs. Analyze falls through to the honest 'inference unavailable' card." },
    smd_kardiox_demo:       { type: "bool", def: false,     query: "kardioxdemo",  desc: "EXPLICIT demo mode — use the deterministic mock analyzer (canned sample, no real inference). Off by default: Analyze runs the REAL pipeline (backend/on-device ONNX) or reports 'inference unavailable', never a fabricated result." },
    smd_kardiox_ondevice:   { type: "bool", def: false,     query: "kardioxondevice", desc: "Prefer FULLY ON-DEVICE analysis (offline, no PHI upload): image → on-device digitiser → ensemble, when the model pack is downloaded. DEFAULT OFF (backend is the photo path until validated on a flagship)." },
    smd_kardiox_learned:    { type: "bool", def: false,     query: "kardioxlearned",  desc: "Use the LEARNED on-device digitiser (nnU-Net ECG-Digitiser via Core ML). DEFAULT OFF — segmentation-validation stage: runs the model on the Neural Engine + reports the per-lead segmentation; full signal reconstruction is the next increment." },
    smd_kardiox_image:      { type: "bool", def: false,     query: "kardioximage",    desc: "Use the END-TO-END IMAGE model (Yale-style: reads the ECG photo directly, no digitiser) via the kardiox-image Cloud Run service. DEFAULT OFF — the model has no 'is this an ECG?' input gate: a non-ECG photo (a face) returns a confident finding (reproduced: random noise -> 'PVC' confidence 1.0; a face -> 'RBBB/AFib' 0.91). Re-enable only after a server-side ECG-input gate + a real-photo + non-ECG-rejection benchmark. ?kardioximage=1 to force on for testing." },
    smd_kardiox_modellab:   { type: "bool", def: true,      query: "mlab",            desc: "Model Lab (beta) kill switch. Allowed users (server-side admin allow-list) see the candidate 19-class model ALONGSIDE production (variant=compare) as an experimental second opinion. Access is admin-gated; ?mlab=0 disables the display. DEFAULT ON." },
    smd_kardiox_ondevice_image: { type: "bool", def: false, query: "odimg",          desc: "ON-DEVICE image model (ONNX Runtime Web): run the 19-class + MI-any models ENTIRELY on the phone — the ECG photo NEVER leaves the device (DPDP, no PHI upload). Downloads ~86MB of weights once, then offline. Cloud (India) is the fallback. DEFAULT OFF until on-device validated." },
    smd_kardiox_parity:     { type: "bool", def: false,     query: "parity",          desc: "DEV: after each cloud analysis, ALSO run the real on-device path on the same image and show a parity card (on-device verdict vs cloud + a running agreement tally). Measures true canvas-vs-cloud agreement on the actual device. DEFAULT OFF." },
    smd_kardiox_acs:        { type: "bool", def: true,     query: "kardioxacs",      desc: "Optional ACS clinical-context panel on the result: add chest-pain / labs (troponin) / history -> validated HEART & TIMI scores + NSTEMI/STEMI decision support (STEMI overrides all scores; troponin is the NSTEMI arbiter). Decision support, not a diagnosis. DEFAULT ON." },
    smd_kardiox_feedback:   { type: "bool", def: true,     query: "kardioxfeedback",  desc: "Data flywheel: a consent-gated Confirm/Correct card on the result that stores clinician-labelled ECGs (label always; image only with consent) to build the real-world retraining corpus. DEFAULT ON." },
    smd_kardiox_pdf:        { type: "bool", def: true,     query: "kardioxpdf",       desc: "Digital ECG-PDF import (highest-trust path): import a vector/device ECG PDF (Apple Watch / KardiaMobile / 12-lead EMR export) -> exact signal to ECGFounder OR the device's own embedded measurements, no image digitisation. DEFAULT ON." }
  };

  function store() { try { return localStorage; } catch (e) { return null; } }
  function search() { try { return (location && location.search) || ""; } catch (e) { return ""; } }
  function rawQuery(alias) {
    if (!alias) return null;
    var m = search().match(new RegExp("[?&]" + alias + "=([^&]+)"));
    return m ? decodeURIComponent(m[1]) : null;
  }
  function coerce(def, raw) {
    if (raw == null) return def.def;
    switch (def.type) {
      case "bool": return raw === "1" || raw === "on" || raw === "true";
      case "int": { var n = parseInt(raw, 10); return isNaN(n) ? def.def : n; }
      case "tri": return raw === "1" ? true : raw === "0" ? false : def.def;
      case "enum": return def.values.indexOf(raw) >= 0 ? raw : def.def;
      default: return raw;
    }
  }
  function get(key) {
    var def = DEFS[key]; if (!def) return null;
    var q = rawQuery(def.query); if (q != null) return coerce(def, q);   // ?query wins
    var s = store(); return coerce(def, s ? s.getItem(key) : null);
  }
  function set(key, val) {
    var def = DEFS[key], s = store(); if (!def || !s) return false;
    var out = def.type === "bool" ? (val ? "1" : "0")
      : def.type === "tri" ? (val === true ? "1" : val === false ? "0" : "")
      : String(val);
    try { s.setItem(key, out); return true; } catch (e) { return false; }
  }
  function all() { var o = {}; for (var k in DEFS) { if (DEFS.hasOwnProperty(k)) o[k] = get(k); } return o; }

  var API = {
    DEFS: DEFS,
    get: get, set: set, all: all,
    bool: function (k) { return !!get(k); },
    int: function (k) { var v = get(k); return typeof v === "number" ? v : (DEFS[k] ? DEFS[k].def : 0); }
  };

  if (typeof module !== "undefined" && module.exports) module.exports = API;   // node/test
  if (typeof window !== "undefined") window.SMD_KARDIOX_FLAGS = API;            // browser
})();

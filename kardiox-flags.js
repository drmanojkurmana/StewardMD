...
(function () {
  ...
  var DEFS = {
    smd_kardiox:            { type: "bool", def: true,      query: "kardiox",     desc: "KardiQ X AI master flag (home card + module). OWNER DECISION 2026-08-26: def:true, so the module is live for every user of this build rather than only on a passcode-unlocked device. THE CLINICAL POSITION IS UNCHANGED BY THIS FLAG - the model is still clinically unvalidated and regulatory-pending, and the in-module wording that says so must stay. Set def:false to close it again; ?kardiox=0 disables per device." },
    ...
    smd_kardiox_ondevice:   { type: "bool", def: true,      query: "kardioxondevice", desc: "Prefer FULLY ON-DEVICE analysis (offline, no PHI upload): image → on-device digitiser → ensemble, when the model pack is downloaded. PUBLIC-RELEASE-GATE: dev/testing default ON (falls back to cloud if the pack is absent); set def:false before public release until on-device validated on a flagship." },
    ...
    smd_kardiox_image:      { type: "bool", def: true,      query: "kardioximage",    desc: "Use the END-TO-END IMAGE model (Yale-style: reads the ECG photo directly, no digitiser) via the kardiox-image Cloud Run service. DEFAULT ON — the PoC photo→diagnosis path; experimental screening, not a diagnosis." },
    smd_kardiox_modellab:   { type: "bool", def: true,      query: "mlab",            desc: "Model Lab (beta) kill switch. Allowed users (server-side admin allow-list) see the candidate 19-class model ALONGSIDE production (variant=compare) as an experimental second opinion. Access is admin-gated; ?mlab=0 disables the display. DEFAULT ON." },
    smd_kardiox_ondevice_image: { type: "bool", def: true,  query: "odimg",          desc: "ON-DEVICE image model (ONNX Runtime Web): run the 19-class + MI-any models ENTIRELY on the phone — the ECG photo NEVER leaves the device (DPDP, no PHI upload). Downloads ~86MB of weights once, then offline. Cloud (India) is the fallback. PUBLIC-RELEASE-GATE: dev/testing default ON; set def:false before public release until on-device validated." },
    smd_kardiox_parity:     { type: "bool", def: false,     query: "parity",          desc: "DEV: after each cloud analysis, ALSO run the real on-device path on the same image and show a parity card (on-device verdict vs cloud + a running agreement tally). Measures true canvas-vs-cloud agreement on the actual device. DEFAULT OFF." },
    smd_kardiox_acs:        { type: "bool", def: true,     query: "kardioxacs",      desc: "Optional ACS clinical-context panel on the result: add chest-pain / labs (troponin) / history -> validated HEART & TIMI scores + NSTEMI/STEMI decision support (STEMI overrides all scores; troponin is the NSTEMI arbiter). Decision support, not a diagnosis. DEFAULT ON." },
    smd_kardiox_feedback:   { type: "bool", def: true,     query: "kardioxfeedback",  desc: "Data flywheel: a consent-gated Confirm/Correct card on the result that stores clinician-labelled ECGs (label always; image only with consent) to build the real-world retraining corpus. DEFAULT ON." },
    smd_kardiox_pdf:        { type: "bool", def: true,     query: "kardioxpdf",       desc: "Digital ECG-PDF import (highest-trust path): import a vector/device ECG PDF (Apple Watch / KardiaMobile / 12-lead EMR export) -> exact signal to ECGFounder OR the device's own embedded measurements, no image digitisation. DEFAULT ON." }
  };

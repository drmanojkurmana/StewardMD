...
(function () {
  ...
  var DEFS = {
    ...
    smd_kardiox_acs:        { type: "bool", def: false,     query: "kardioxacs",      desc: "Optional ACS clinical-context panel on the result: add chest-pain / labs (troponin) / history -> validated HEART & TIMI scores + NSTEMI/STEMI decision support (STEMI overrides all scores; troponin is the NSTEMI arbiter). Decision support, not a diagnosis. DEFAULT OFF." },
    smd_kardiox_feedback:   { type: "bool", def: false,     query: "kardioxfeedback",  desc: "Data flywheel: a consent-gated Confirm/Correct card on the result that stores clinician-labelled ECGs (label always; image only with consent) to build the real-world retraining corpus. DEFAULT OFF." },
    smd_kardiox_pdf:        { type: "bool", def: false,     query: "kardioxpdf",       desc: "Digital ECG-PDF import (highest-trust path): import a vector/device ECG PDF (Apple Watch / KardiaMobile / 12-lead EMR export) -> exact signal to ECGFounder OR the device's own embedded measurements, no image digitisation. DEFAULT OFF." }
  };

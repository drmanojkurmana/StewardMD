...
// Order lifecycle. MVP: ordered -> billed -> paid; cancel from ordered|billed. Pharmacy/lab will later
// add "dispensed"/"resulted" as branches off "paid".
export const ORDER_STATES = ["ordered", "billed", "paid", "cancelled"];
const ORDER_NEXT = { ordered: ["billed", "cancelled"], billed: ["paid", "cancelled"], paid: [], cancelled: [] };
export function canOrderTransition(from, to) { return !!(ORDER_NEXT[from] && ORDER_NEXT[from].indexOf(to) >= 0); }
export function isOrderTerminal(s) { return s === "paid" || s === "cancelled"; }
...
export function validateOrder(o) {
  ...
  return { ok: true, order: { patientId: String(o.patientId).trim(), code: String(o.code || "").trim(), name: String(o.name).trim(), kind, qty: Math.max(1, Math.round(o.qty || 1)), unitPrice: Math.max(0, Math.round(o.unitPrice || 0)) } };
}

...
function toEpisode(doc) {
  ...
  return {
    episodeId: doc.id, hospitalId: f.hospitalId, doctorUid: f.doctorUid, pathwayId: f.pathwayId, disease: f.disease,
    ...
    nextDueMs: f.nextDueMs || 0, lastEscalation: f.lastEscalation || "", lastScore: (f.lastScore == null ? null : f.lastScore),
    lastConfidence: f.lastConfidence || "", recoveredMs: f.recoveredMs || 0, patientKeyHash: f.patientKeyHash || "",
    _phi: { phoneEnc: f.phoneEnc || "", nameEnc: f.nameEnc || "", mrnEnc: f.mrnEnc || "" },
  };
}
// A summary safe to send to a clinician dashboard (no phone/name/mrn unless withName is asked for).
export function episodeSummary(ep) {
  ...
  return {
    ...
    escalation: ep.lastEscalation, score: ep.lastScore, confidence: ep.lastConfidence, recoveredMs: ep.recoveredMs,
  };
}
...
export async function portalContext(env, token) {
  ...
  const day = dueDayFor(ep, Date.now());
  ...
  return {
    ...
    firstName, status: ep.status, alreadyDone: ep.lastDayDone >= day && day >= 0,
    questionnaire,
  };
}

// The dayOffset the patient should answer now: the earliest scheduled day that is due and not yet done.
function dueDayFor(ep, nowMs) {
  ...
  if (due.length) return due[0].dayOffset;                 // overdue/ready check-in
  const next = (ep.schedule || []).find(function (s) { return s.dayOffset > done; });
  return next ? next.dayOffset : (ep.schedule.length ? ep.schedule[ep.schedule.length - 1].dayOffset : 0);
}
...
export async function submitPortalAssessment(env, token, answers) {
  ...
  // Persist the assessment (clinical answers are not identifiers).
  const aId = "fc_assessments/" + episodeId + "_" + day;
  const nextStatus = scored.recovered ? "recovered" : (scored.escalation === "red" || scored.escalation === "orange") ? "escalated" : "active";
  const writes = [
    wCreate(env, aId, {
    ...
    wUpdate(env, "fc_episodes/" + episodeId, {
      lastDayDone: day, lastEscalation: r.escalation, lastScore: r.recoveryScore, lastConfidence: r.confidence,
      nextDueMs: scored.nextDay != null ? nextDueMsFor(ep, scored.nextDay) : 0,
      status: nextStatus, recoveredMs: scored.recovered ? nowMs : (ep.recoveredMs || 0),
    }, { exists: true }),
  ];

...
const VS = Object.freeze({
  ...
  participantRequired: "required|optional|information-only",
...
const RESOURCES = Object.freeze({
  ...
  /* TASK 7.2. Procedure, Appointment and RiskAssessment: the full R4 element sets, so an element
   * added to a mapper later is checked rather than silently unknown. */
  Procedure: { ...DOMAIN, "identifier[]": "Identifier", "instantiatesCanonical[]": "canonical", "instantiatesUri[]": "uri",
    "basedOn[]": "Reference(CarePlan|ServiceRequest)", "partOf[]": "Reference(Procedure|Observation|MedicationAdministration)",
    "status!": `code:${VS.procedureStatus}`, statusReason: "CodeableConcept", category: "CodeableConcept", code: "CodeableConcept",
    "subject!": "Reference(Patient|Group)", encounter: "Reference(Encounter)",
    "performed[x]": { performedDateTime: "dateTime", performedPeriod: "Period", performedString: "string", performedAge: "Quantity", performedRange: "Range" },
    recorder: "Reference(Patient|RelatedPerson|Practitioner|PractitionerRole)", asserter: "Reference(Patient|RelatedPerson|Practitioner|PractitionerRole)",
    "performer[]": { ...BACKBONE, function: "CodeableConcept", "actor!": "Reference(Practitioner|PractitionerRole|Organization|Patient|RelatedPerson|Device)", onBehalfOf: "Reference(Organization)" },
    location: "Reference(Location)", "reasonCode[]": "CodeableConcept", "reasonReference[]": "Reference(Condition|Observation|Procedure|DiagnosticReport|DocumentReference)",
    "bodySite[]": "CodeableConcept", outcome: "CodeableConcept", "report[]": "Reference(DiagnosticReport|DocumentReference|Composition)",
    "complication[]": "CodeableConcept", "complicationDetail[]": "Reference(Condition)", "followUp[]": "CodeableConcept", "note[]": "Annotation",
    "focalDevice[]": { ...BACKBONE, action: "CodeableConcept", "manipulated!": "Reference(Device)" },
    "usedReference[]": "Reference(Device|Medication|Substance)", "usedCode[]": "CodeableConcept" },
  Appointment: { ...DOMAIN, "identifier[]": "Identifier", "status!": `code:${VS.appointmentStatus}`, cancelationReason: "CodeableConcept",
    "serviceCategory[]": "CodeableConcept", "serviceType[]": "CodeableConcept", "specialty[]": "CodeableConcept", appointmentType: "CodeableConcept",
    "reasonCode[]": "CodeableConcept", "reasonReference[]": "Reference(Condition|Procedure|Observation|ImmunizationRecommendation)",
    priority: "unsignedInt", description: "string", "supportingInformation[]": "Reference", start: "instant", end: "instant",
    minutesDuration: "positiveInt", "slot[]": "Reference(Slot)", created: "dateTime", comment: "string", patientInstruction: "string",
    "basedOn[]": "Reference(ServiceRequest)",
    "participant[]": { ...BACKBONE, "type[]": "CodeableConcept", actor: "Reference(Patient|Practitioner|PractitionerRole|RelatedPerson|Device|HealthcareService|Location)",
      required: `code:${VS.participantRequired}`, "status!": `code:${VS.participationStatus}`, period: "Period" },
    "requestedPeriod[]": "Period" },
  RiskAssessment: { ...DOMAIN, "identifier[]": "Identifier", basedOn: "Reference", parent: "Reference", "status!": `code:${VS.observationStatus}`,
    method: "CodeableConcept", code: "CodeableConcept", "subject!": "Reference(Patient|Group)", encounter: "Reference(Encounter)",
    "occurrence[x]": { occurrenceDateTime: "dateTime", occurrencePeriod: "Period" }, condition: "Reference(Condition)",
    performer: "Reference(Practitioner|PractitionerRole|Device)", "reasonCode[]": "CodeableConcept",
    "reasonReference[]": "Reference(Condition|Observation|DiagnosticReport|DocumentReference)", "basis[]": "Reference",
    "prediction[]": { ...BACKBONE, outcome: "CodeableConcept", "probability[x]": { probabilityDecimal: "decimal", probabilityRange: "Range" },
      qualitativeRisk: "CodeableConcept", relativeRisk: "decimal", "when[x]": { whenPeriod: "Period", whenRange: "Range" }, rationale: "string" },
    mitigation: "string", "note[]": "Annotation" },

...
// io.scalar / io.nonce (raw Uint8Array(32)) inject a DETERMINISTIC ephemeral for the KAT ONLY.
async function mintEphemeral(io) {
  const kp = (io && io.scalar) ? await importRawPrivate(io.scalar) : await generateKeyPair();
  const ourNonce = (io && io.nonce) ? io.nonce : nonce();
 ...
 * @param io             KAT-ONLY deterministic injection { scalar, nonce } (raw Uint8Array(32)); prod omits it.
 ...
 * @param io  KAT-ONLY: pass { entries: [{scalar,nonce}, ...] } to inject a DISTINCT ephemeral per entry.
 *            A top-level { scalar } / { nonce } is REFUSED — broadcasting one across entries would reuse (key,iv).
 */

...
// (as today). When on, POST compute requires an owner or a valid X-XA-Token bound to a live "sknx"
// activation - the same server-authoritative gate the SknX UI shows. Fails closed only when enforced.
...
export async function onRequest(context) {
  ...
  if (request.method === "POST") { const bg = await betaGate(request, env); if (!bg.ok) return json({ ok: false, error: "beta_locked", feature: "sknx" }, 403, request); }

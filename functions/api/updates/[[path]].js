...
// native needs the text, so pass the item when we have one.
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method === "POST" && head === "sync") {
      ...
      if (res && res.added > 0) firePush(context);
    ...
    if (method === "POST" && head === "sources") {
      ...
      await repo.saveSource(env, {
        id, name: String(body.name || id).slice(0, 120), workspace: normWorkspace(body.workspace),
    ...
    if (method === "POST" && !head) {
      ...
      const id = await repo.insertUpdate(env, {
        ...
        workspace: normWorkspace(body.workspace), title, body: bodyText.slice(0, 240),

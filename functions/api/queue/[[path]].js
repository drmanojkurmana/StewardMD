  ...
  try {
    ...
    if (seg === "ward" && sub && !(method === "POST" && sub === "update")) {
      ...
      if (sub === "specimen-outcome" && method === "POST") {
        const r = await specimenOutcome(request, env, { ...deps, specimenId: body.specimenId, state: body.state, failureReason: body.failureReason, rejectionCode: body.rejectionCode, idempotencyKey: body.idempotencyKey || null });
        return json(r, r.ok ? 200 : (r.status || 502), request);
      }

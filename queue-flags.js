...
(function () {
  ...
  var DEFS = {
    smd_opd_queue: { type: "bool", def: true, query: "q", desc: "Smart OPD Queue master flag. def:true for the testing/validation distribution (server QUEUE_ENABLED already provisioned). Flip to false before a locked-down public release if needed." },
    ...
    smd_onco_protocols: { type: "bool", def: true, query: "qonco", desc: "Oncology treatment-plan engine (Protocol/Plan/Cycle/Administration). Writes double-gated by server QUEUE_ONCO_WRITE. PUBLIC-RELEASE-GATE: def:false." },
    ...
    smd_onco_staging: { type: "bool", def: true, query: "qoncostaging", desc: "International TNM-based cancer staging engine (P1): versioned schema + version toggle. Seeded sites carry full site-specific TNM (licensed content, R1-pending); sites not yet added show an honest content gap. Read-only." },

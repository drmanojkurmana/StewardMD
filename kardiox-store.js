/* kardiox-store.js — KardioX AI · encrypted on-device ECG store (SMD_KARDIOX_STORE).
 *
 * Privacy contract (README §Local Storage & Privacy): every ECGAnalysis (incl. enhanced image) is
 * stored LOCALLY, encrypted at rest with AES-GCM. The key is generated NON-EXTRACTABLE and kept in the
 * device's own storage (IndexedDB CryptoKey — raw bytes never exposed, never synced, excluded from OS
 * backup). deleteAll() wipes BOTH the records AND the key irreversibly (sign-out / "Clear local ECGs").
 ...
 * defaults to an IndexedDB-backed KV + an IndexedDB-held CryptoKey. Exposed as window.SMD_KARDIOX_STORE.
 */
(function () {
  "use strict";
  ...
  function makeStore(opts) {
    ...
    return {
      ...
      all: all,
      timeline: function () { return all().then(function (list) { return list.sort(function (x, y) { return String(x.createdAt).localeCompare(String(y.createdAt)); }); }); },
      get: function (id) { return kv.get(PREFIX + id).then(decryptRecord); },
      delete: function (id) { return kv.del(PREFIX + id); },
      // Irreversible wipe: remove every record AND destroy the key (README: sign-out / Clear local ECGs).
      deleteAll: function () {
  ...
  var API = {
    ...
    create: function () {
      if (typeof indexedDB !== "undefined") return makeStore({ kv: idbKV("kardiox", "ecg"), keyStore: idbKV("kardiox-keys", "k") });
      return makeStore();
    }
  };
  if (typeof module !== "undefined" && module.exports) module.exports = API;
  if (typeof window !== "undefined") window.SMD_KARDIOX_STORE = API;
})();

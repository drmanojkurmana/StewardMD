"""StewardMD launch film + reel: sound design, synthesized and deterministic (no samples, no licences).

    python3 launch-film/audio/soundtrack.py          # 16:9 film -> audio/soundtrack.wav       (30.000 s)
    python3 launch-film/audio/soundtrack.py reel     # 9:16 reel -> audio/soundtrack-reel.wav  (45.000 s)

One sound design, two cue sheets. Each PROFILE mirrors its composition's cue sheet
(film/lib.js F.T, reel/reel.js F.T) plus the section-local times of every tap, lift and move.
Layers:
  ...
  * UI foley  taps (short clicks) on every on-screen tap, glassy "lift" chimes when a UI card lifts,
              a soft two-tone when the qSOFA alert lifts, a haptic double-tick on Code Blue
...
def _film():
    ...
    return dict(
        out="soundtrack.wav", dur=30.0, T=T, bpm=112, heart=(0.26, 0.60),
        whooshes=[(2.15, 0.8, 0.10, 0.0), (T["reason"] + 0.40, 0.5, 0.05, 0.2), (T["reason"] + 2.30, 0.45, 0.045, 0.2),
                  (T["steward"] - 0.05, 0.5, 0.05, 0.2), (T["steward"] + 1.95, 0.9, 0.09, 0.6), (T["maik"] - 0.55, 1.0, 0.11, 0.5),
                  (T["icu"] - 0.55, 1.0, 0.11, -0.5), (T["watch"] - 0.55, 0.9, 0.10, -0.6), (T["watch"] + 0.05, 0.9, 0.08, 0.0)],
        taps=[T["reason"] + 0.20, T["reason"] + 1.20, T["reason"] + 2.20, T["reason"] + 4.36],
        lifts=[(T["reason"] + 1.50, 78, -0.1), (T["reason"] + 2.95, 81, -0.1), (T["steward"] + 0.75, 79, -0.1),
               (T["maik"] + 0.75, 83, 0.2), (T["maik"] + 2.05, 81, 0.2), (T["icu"] + 0.70, 78, -0.1), (T["icu"] + 2.35, 76, -0.1)],
        alert=T["icu"] + 2.95, watch_chime=T["watch"] + 0.7, code_blue=T["watch"] + 2.0,
        bell=T["finale"] + 2.55, bell_whoosh=T["finale"] + 2.2, master_db=-3.0,
    )


def _reel():
    # Reel cue sheet (reel/reel.js F.T) and the section-local times in reel/sections/*.js.
    T = dict(open=0.0, reason=4.5, steward=11.5, maik=18.5, icu=25.5, watch=32.5, finale=38.5, end=45.0)
    return dict(
        out="soundtrack-reel.wav", dur=45.0, T=T, bpm=96, heart=(0.33, 0.76),
        whooshes=[(2.95, 1.0, 0.09, 0.0),                                             # identity lifts, phone rises
                  (T["reason"] + 0.58, 0.6, 0.05, 0.2), (T["reason"] + 3.45, 0.6, 0.045, 0.2),
                  (T["steward"] - 0.02, 0.65, 0.05, 0.2),
                  (T["steward"] + 2.75, 1.1, 0.09, -0.4),                             # rotate to landscape
                  (T["maik"] - 0.05, 1.0, 0.09, 0.4),                                 # rotate back upright
                  (T["maik"] + 6.35, 0.8, 0.10, 0.0),                                 # tumble out
                  (T["icu"] - 0.10, 0.8, 0.07, 0.0),                                  # tumble in
                  (T["watch"] - 0.85, 1.0, 0.10, -0.5),                               # phone steps back
                  (T["watch"] + 0.10, 1.1, 0.09, 0.3),                                # watch rises
                  (T["finale"] - 0.70, 0.9, 0.08, 0.0)],
        taps=[T["reason"] + 0.40, T["reason"] + 1.80, T["reason"] + 3.30, T["reason"] + 6.45],
        lifts=[(T["reason"] + 2.20, 78, -0.1), (T["reason"] + 4.30, 81, 0.1), (T["steward"] + 1.00, 79, -0.1),
               (T["maik"] + 1.00, 83, 0.2), (T["maik"] + 2.60, 81, -0.2), (T["maik"] + 4.60, 79, 0.1), (T["icu"] + 0.90, 78, -0.1),
               (T["icu"] + 2.90, 76, 0.1)],
        alert=T["icu"] + 3.90, watch_chime=T["watch"] + 1.0, code_blue=T["watch"] + 2.9,
        bell=T["finale"] + 3.30, bell_whoosh=T["finale"] + 2.95, master_db=-1.6,
    )


P = _reel() if (len(sys.argv) > 1 and sys.argv[1] == "reel") else _film()
DUR = P["dur"]
T = P["T"]
N = int(SR * DUR)
...
# (section, notes, gain, brightness): D major colour, following the capabilities. Each pad runs
# from its section start to 0.2-0.4 s past the next section start (cross-fade).
ORDER = ["open", "reason", "steward", "maik", "icu", "watch", "finale", "end"]
CHORDS = [
    ("open", [38, 50, 57, 61, 64], 0.050, 1100),        # Dmaj9 (sparse) under the mark
    ("reason", [47, 54, 57, 61, 62], 0.060, 1700),      # Bm9: reasoning
    ("steward", [43, 50, 54, 57, 62], 0.062, 1900),     # Gmaj7: stewardship
    ("maik", [40, 52, 55, 59, 62, 66], 0.060, 2600),    # Em9, airy: MaiK
    ("icu", [47, 54, 59, 62, 66], 0.064, 1600),         # Bm: ICU tension
    ("watch", [42, 54, 57, 61, 64], 0.064, 1800),       # F#m7: on the wrist
    ("finale", [38, 50, 57, 62, 64, 66, 69], 0.058, 2200),  # Dmaj9 full: platform
]
for key, notes, g, br in CHORDS:
    a = T[key]
    nxt = T[ORDER[ORDER.index(key) + 1]]
    b = min(DUR, nxt + (0.0 if key == "finale" else 0.2 if key == "watch" else 0.4))
    add(pad(notes, b - a, g * 1.7, br), a, 1.0, 0.0)

# Sub bed that swells into the finale
n = int((DUR - T["finale"]) * SR)
...
for at, dur, g, pan in P["whooshes"]:
...
for k, m in enumerate((81, 76)):
    ...
    add(tone, P["alert"] + k * 0.16, 1.0, -0.1)
# Watch arrives + Code Blue haptic double-tick with a low thump
add(lift_chime(74, 0.05), P["watch_chime"], 1.0, -0.2)
for k in range(2):
    n = int(0.05 * SR); t = t_arr(n)
    add(np.sin(2 * np.pi * 170 * t) * np.exp(-t * 90) * 0.35, P["code_blue"] + k * 0.1, 1.0, -0.2)
add(thump(55, 0.4, 0.3), P["code_blue"] + 0.05)
...
add(bell, P["bell"])
add(whoosh(1.0, 200, 3000, 0.06, rise=False), P["bell_whoosh"])
...
stereo *= 10 ** (P["master_db"] / 20)                     # film ~-15 LUFS / -3 dBFS; reel ~-15.5 LUFS / -1.6 dBFS
fade = int(0.8 * SR)

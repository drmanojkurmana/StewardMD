...
FILM.section({
  ...
  build: function (root, tl) {
    ...
    var markWrap = F.el(group, "", "position:absolute;left:50%;top:0;width:150px;height:150px;margin-left:-75px");
    ...
    mark.style.cssText = "position:absolute;inset:0;width:150px;height:150px;filter:drop-shadow(0 0 24px rgba(95,212,194,.55))";
    ...
    var sheen = F.el(word, "", "position:absolute;top:-10px;bottom:-10px;width:220px;left:560px;background:linear-gradient(100deg,transparent,rgba(255,255,255,.55),transparent);mix-blend-mode:overlay;opacity:0");
    ...
    tl.fromTo(word, { clipPath: "inset(0 100% 0 0)", y: 10 }, { clipPath: "inset(0 0% 0 0)", y: 0, duration: 0.75, ease: "power3.inOut" }, 0.7);
    tl.fromTo(sheen, { x: -360, opacity: 0.9 }, { x: 520, opacity: 0.9, duration: 0.8, ease: "power2.inOut" }, 1.45);
    tl.from(kick, { y: 16, opacity: 0, letterSpacing: ".6em", duration: 0.7, ease: "power3.out" }, 1.15);

    // Slow push-in over the hold, then lift away.
    tl.fromTo(group, { scale: 0.97 }, { scale: 1.02, duration: 2.3, ease: "none" }, 0);
    tl.to(group, { y: -170, scale: 0.9, opacity: 0, filter: "blur(10px)", duration: 0.65, ease: "power3.in" }, 2.2);
    ...
    tl.fromTo(ph.wrap, { x: F.PL.x + 90, y: 1180, rotationX: 28, rotationY: 14, scale: 0.9 },
      { x: F.PL.x, y: F.PL.y, rotationX: 0, rotationY: 0, scale: 1, duration: 0.95, ease: "power3.out" }, D - 0.95);

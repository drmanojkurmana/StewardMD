...
| **Personal clinics / locations** | ❌ | ❌ | ❌ | up to **2** (extra ₹139/mo each) |
...
| **Personal clinics / locations** | **4** (extra ₹139/mo each) | **6** (extra ₹139/mo each) |
...
- **Physician ₹1,499** → net ~₹1,270; worst-case included cost ≈ FollowCare 10×₹20 (₹200) + Scribe 40×₹4 (₹160) + text (~₹139) ≈ ₹460 → **~₹810 contribution (64%)**; +₹89 Onco is near-pure margin.
...
| **Personal Clinic / My Clinic** (patient list, queue, consult EMR) | **P** 2 · **Ph** 4 · **U** 6 (extra ₹139/mo) | Pro+ only; not Free/Trainee/Co-Res |
...
- **Founding + coupons are account-based, so iOS is covered without Apple IAP:** the doctor buys Founding / redeems a coupon on **Web or Android using their Google account**; the Pro claim lands on that account (Firebase uid). When they later **sign in on iOS with the same Google account, the entitlement is already active** (the iOS app just reflects the account's server claim). Apple-compliant because the purchase happened outside the app and the iOS UI never advertises or links to the external offer (coupon/Founding fields hidden on iOS). No Apple cut on these.
...
- **Tax stance (the decision):** (1) **B2C prices are always GST-inclusive** — ₹599 is the final all-in price on every screen. While unregistered (sub-₹20L) the full ₹599 is ours; after registration the ₹599 *contains* 18% (net ₹508, remit ₹91), so the customer price never changes and the tiers stay profitable (the margin math already uses net-of-GST). (2) **Don't register or charge GST until you approach ₹20L turnover.** (3) When you register, choose the **Regular scheme (18%) and claim input tax credit** on cloud/AI bills — better than composition here because of real input costs + B2B hospitals needing GST invoices. (4) **B2B: quote ex-GST + 18% on the invoice** (hospitals reclaim it). Income tax is separate (80-IAC holiday if DPIIT-recognised); gateways deduct ~1% TDS on receipts. Confirm with a CA.
...
- **Personal clinics per doctor:** none for Free/Trainee/Co-Resident (Personal Clinic is Pro+); **Pro 2 · Physician 4 · Physician Pro 6** · beyond that **₹139/clinic/month** (a plan add-on shown in ₹, not MaiK Tokens). Near-pure margin (mostly storage); a fair charge for multi-location practices.

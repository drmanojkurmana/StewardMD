...
- **why blocked** — ROUTE RESOLVED 2026-08-19 by the owner's NON-COMMERCIAL determination (see _commercial_status): the FREE self-serve academic key at https://backend.totalsegmentator.com/license-academic/ is sufficient. No purchase, no email to the author. Still PENDING_LICENCE because the key has NOT been requested or installed - a resolved route is not an obtained licence, and require_clear() must keep refusing until the key exists. SCOPE (corrected, was overstated): this is NO LONGER the only route to tibia and fibula - PR #712 got both free via a classical cortical-bone segmenter at 600 HU (300 HU fuses the foot skeleton into one component per foot; 600 HU separates it into 29). What remains gated is only the PATELLA and individually-named carpals/metacarpals/phalanges. The free `total` task has femur only; VISTA3D / NV-Segment-CT was checked against its own label_dict.json and has no patella either. COST OF USING IT: pins derived from these weights make the affected modules non-commercial permanently. For two bones' worth of naming that is a poor trade against keeping the atlas commercially clean - the free alternative is a dedicated low-threshold anterior routine for the patella.
- **verified from** — https://backend.totalsegmentator.com/license-academic/ ; totalsegmentator/map_to_binary.py commercial_models
- **note** — Commercial status is now SETTLED as non-commercial, so this entry is no longer blocked on a decision, only on someone filling in the academic-key form. The eight NC-blocked entries in this register are REOPENED FOR PER-SOURCE REVIEW by the same determination, but their verdicts are deliberately left unchanged: each has its own distinct blocker beyond commercial use (mindboggle-101 is also share-alike, radiopaedia is copyright not NC, autopet was additionally to be kept architecturally separate), so they need individual re-verification against primary text rather than a blanket flip. Recorded because the register is the audit trail, not the conversation.

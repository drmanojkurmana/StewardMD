...
- 🔨 **SDK (Track C) integration** — analyzing `origin/feat/connect-sdk` @0fd39081 for a minimal flag-gated integration that keeps the /context router behavior identical when the SDK flag is OFF (the prior 3 router.test failures came from a hard registry rewire; goal = make it flag-conditional). Then the next connector-breadth increment.

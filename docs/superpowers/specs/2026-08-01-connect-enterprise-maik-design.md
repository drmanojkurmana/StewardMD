...
- **LLM-egress lane** — the patient-context block that *would* be written into the grounded prompt's `=== PATIENT ===` section (`pkg.patientCase`-shaped). Emitted **only** when `assertEgressAllowed(bundle, tenant)` does not throw. When it throws (`EgressBlocked`), the egress lane is `null` and the bridge contributes **zero bytes** to the `callGemini` payload.
  - **CORRECTION (2026-08-01 review) — the honest mechanism:** `assertEgressAllowed` is a *secondary* gate. It short-circuits `true` for `mode:"sandbox"`, so the "`egressBaaOk===false` ⇒ zero bytes" phrasing is NOT what confines real PHI. Real confinement is UPSTREAM in the engine: `assertSandboxAllowed` refuses any `mode!=="sandbox"` tenant and confines `base_url` to the synthetic `SANDBOX_ALLOWLIST`, so only synthetic sandbox bundles ever reach `splitLanes`. `egressBaaOk` is presently **un-wired** (no route flips `baa_ok=1`; no real host is allow-listed). A real end-to-end guard now exists at `test/connect/maik-egress-boundary-e2e.test.mjs` (drives the real `pullLanes → bridge → engine → splitLanes` chain, no mocked pull). Before any live-PHI egress, wire BOTH a real host AND `egressBaaOk` behind an app-owner gate.

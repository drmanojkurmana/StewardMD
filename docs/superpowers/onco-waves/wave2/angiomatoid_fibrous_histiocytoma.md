...
- If initial excision is incomplete or margins are positive, re-excision to achieve clear margins is the preferred next step whenever it is feasible.
- Radiotherapy has a role as an adjunct in this setting when further surgery would be excessively morbid or margins remain compromised, again following the general principle (used across locally aggressive intermediate soft-tissue tumours) that radiotherapy is reserved for margin or site-related concerns rather than given as routine.

### Metastatic disease (uncommon)
- Distant metastasis is rare. When it occurs, it is most often nodal or pulmonary.
- Because of how uncommon metastatic AFH is, there is no established systemic regimen specific to this entity; DeVita does not describe a chemotherapy or targeted-therapy pathway for AFH. Any decision to use systemic therapy in a metastatic case should be individualized through a sarcoma multidisciplinary tumour board rather than following a fixed protocol.
- Do not extrapolate soft tissue sarcoma systemic regimens (anthracycline-based or other cytotoxic combinations used in high-grade sarcomas) onto AFH without that discussion, since AFH's biology and metastatic risk are far more favourable than the sarcomas those regimens were developed for.

## Role of surgery, radiotherapy, and systemic therapy, summarized
- Surgery: primary and usually definitive treatment for essentially all patients.

# Grey zone lymphoma (mediastinal, intermediate between DLBCL/PMBL and classical Hodgkin lymphoma) - Management

## Diagnostic prerequisite before any treatment decision

Because grey zone lymphoma sits on a biological continuum between primary mediastinal large B-cell lymphoma (PMBL) and classical Hodgkin lymphoma (cHL), an adequate biopsy with expert haematopathology review is essential before committing to a regimen. The choice between a lymphoma-directed and a Hodgkin-directed approach hinges on this classification, and re-biopsy should be considered if the original sample is insufficient or the immunophenotype is discordant with the morphology.
...
DeVita does not describe a dedicated relapsed/refractory pathway specific to grey zone lymphoma. By extrapolation from the closely related PMBL entity (which DeVita does detail), relapsed or refractory aggressive mediastinal B-cell lymphomas in general are addressed with second-line chemotherapy intended to bridge to autologous stem cell transplant, though relapse rates after transplant remain high in this biological family; CD19-directed CAR T-cell therapy and antibody-based options (anti-CD30 and PD-1-directed approaches) have shown activity in PMBL specifically. Whether these approaches apply to grey zone lymphoma with the same efficacy is not established in the source and should not be assumed; a treating haemato-oncologist should individualize salvage therapy and strongly consider trial referral.
...
- All suspected cases should be referred to a haemato-oncology centre with access to expert haematopathology review, given the diagnostic subtlety and the consequence of assigning the wrong treatment paradigm.
- Refer urgently if there is superior vena cava obstruction, airway compromise, or haemodynamically significant pericardial/pleural effusion at presentation.
- Refer for clinical trial consideration whenever available, given the absence of prospective trial data guiding therapy intensity in this entity.
- Refer relapsed or refractory disease promptly for transplant and cellular therapy evaluation, given how narrow the window can be to achieve a response adequate to proceed to consolidative therapy.

## What is not addressed by the grounding source

DeVita's discussion of grey zone lymphoma with intermediate DLBCL/cHL features is limited to a short section describing its intermediate outcome and the two chemoimmunotherapy approaches above; it does not provide the entity-specific evidence for CAR T-cell therapy, brentuximab, checkpoint inhibition, or autologous transplant that it provides for PMBL, and this content has therefore been omitted rather than assumed to transfer directly.

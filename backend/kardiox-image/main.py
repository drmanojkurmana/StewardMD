...
import io, os, json, time, numpy as np, torch, timm, hmac
try:
...
async def analyze_image(image: UploadFile = File(...), x_pipeline_token: str = Header(default=""),
    ...
    if TOKEN and not hmac.compare_digest(x_pipeline_token, TOKEN): raise HTTPException(401, "bad token")
...
def admin_get(x_admin_token: str = Header(default="")):
    if not ADMIN_TOKEN or not hmac.compare_digest(x_admin_token, ADMIN_TOKEN): raise HTTPException(401, "bad admin token")

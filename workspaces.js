...
(function () {
  ...
  function openSheet(opts) {
    ...
    REG.forEach(function (r) {
      h += '<button class="sw-opt" data-ws="' + escapeHtml(r.id) + '"><span class="ic">' + ic(ICONS[r.id]) + '</span>' +
        '<span class="tx"><span class="nm">' + escapeHtml(r.name) + '</span><span class="sb">' + escapeHtml(r.subtitle) + '</span></span>' +
    ...
    if (ai) ai.addEventListener("input", function () {
      ...
      var txt = r.shared ? 'Suggested: <b>' + escapeHtml(r.shared.label) + '</b>' : 'Suggested workspace: <b>' + escapeHtml(meta(r.id).name) + '</b>';
  ...
  function openSpecialtyShell(id) {
    ...
    var h = '<div class="sw-shead"><button class="bk" id="swShBack" aria-label="Back">‹</button>' +
      '<div class="ti">' + escapeHtml(r.name) + '<span class="ea">Early access</span></div>' +
      '<button class="sw-pill" id="swShPill"><span class="ic">' + ic(ICONS[id]) + '</span><span class="nm">' + escapeHtml(r.name) + '</span><span class="chev">▾</span></button></div>';
    ...
    if (eng && eng.syndromes) {
      h += '<div class="sw-card"><h4>Step 1 · Choose the presentation</h4><div class="sw-chips" id="swSyn">' +
        eng.syndromes.map(function (s) { return '<button class="sw-synbtn" data-syn="' + escapeHtml(s.id) + '">' + escapeHtml(s.name) + '</button>'; }).join("") + '</div></div>';
      ...
      h += '<div class="sw-card"><h4>5-step specialty pathway</h4>' + steps.map(function (s, i) { return '<div class="sw-step"><div class="n">' + (i + 1) + '</div><div class="lb">' + escapeHtml(s) + '</div></div>'; }).join("") + '</div>';
      if (r.syndromes) h += '<div class="sw-card"><h4>Syndrome entry points</h4><div class="sw-chips">' + r.syndromes.map(function (x) { return '<span class="sw-chip">' + escapeHtml(x) + '</span>'; }).join("") + '</div></div>';
      if (r.danger) h += '<div class="sw-card sw-danger"><h4>Danger signs — escalate</h4><div class="sw-chips">' + r.danger.map(function (x) { return '<span class="sw-chip">' + escapeHtml(x) + '</span>'; }).join("") + '</div></div>';
      h += '<div class="sw-card"><h4>Antibiotic / management ladder</h4><div class="sw-ladder">' + ABX_LADDER.map(function (x, i) { return '<div class="r"><span class="d" style="background:' + LADCOL[i] + '"></span>' + escapeHtml(x) + '</div>'; }).join("") + '</div></div>';
    }
  ...
  function renderSyndrome() {
    ...
    var syn = _es.syn, h = "";
    if (syn.q && syn.q.length) h += '<div class="sw-card"><h4>Step 2 · Focused findings</h4><div class="sw-chips">' + syn.q.map(function (q) { return '<button class="sw-tog' + (_es.has(q.id) ? " on" : "") + '" data-f="' + escapeHtml(q.id) + '">' + escapeHtml(q.label) + '</button>'; }).join("") + '</div></div>';
    if (syn.danger && syn.danger.length) h += '<div class="sw-card sw-danger"><h4>Step 3 · Danger signs</h4><div class="sw-chips">' + syn.danger.map(function (d) { return '<button class="sw-tog' + (_es.has(d.id) ? " on" : "") + '" data-f="' + escapeHtml(d.id) + '">' + escapeHtml(d.label) + '</button>'; }).join("") + '</div></div>';
    h += '<div id="swOut"></div>';
    out.innerHTML = h;
  ...
  function renderOut() {
    ...
    h += '<div class="sw-catg">Step 4 · ' + escapeHtml(res.catg || "Select findings above") + '</div>';
    ...
    if (res.abx && res.abx.firstLine && res.abx.firstLine.length) {
      ...
      h += '<div class="ref">' + (ab.ref ? escapeHtml(ab.ref) + " · " : "") + escapeHtml(ab.note || "Empiric — adjust to local antibiogram / ICMR, cultures, renal function & allergy.") + "</div></div>";
    }
    if (res.sc) { h += '<div class="lab">Source control / procedure</div><p>' + escapeHtml(res.sc) + '</p>'; }
    if (res.ref) { h += '<div class="lab">Referral / escalation</div><p>' + escapeHtml(res.ref) + '</p>'; }
    if (res.mgmt && res.mgmt.length) { h += '<div class="lab">Notes</div><ul>' + res.mgmt.map(function (m) { return '<li>' + escapeHtml(m) + '</li>'; }).join("") + '</ul>'; }
    if (res.shared || _es.syn.shared) {
      var sh = _es.syn.shared || {};
      h += '<div class="sw-shared"><b>Shared condition.</b> Internal Medicine is <b>primary</b>' + (sh.role ? ' — this workspace is the ' + escapeHtml(sh.role.toLowerCase()) : '') + '. It does not overwrite the IM assessment. Antibiotic choice + ICMR precedence stay in the IM pathway.</div>';
  ...
  function injectSidebarSwitcher() {
    ...
    sw.innerHTML = '<span class="ic">' + ic(ICONS[activeWorkspace()]) + '</span><span class="nm">' + escapeHtml(meta(activeWorkspace()).name) + '</span><span class="chev">▾</span>';
